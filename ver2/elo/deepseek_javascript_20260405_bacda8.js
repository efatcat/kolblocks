...
let playerName = localStorage.getItem('kolblocks_player_name') || '';
...
function showNameModal() {
    ...
    const saveName = () => {
        ...
        if (newName === '') newName = 'Игрок';
        if (newName.length > 20) newName = newName.substring(0, 20);
        playerName = newName;
        localStorage.setItem('kolblocks_player_name', playerName);
        modal.style.display = 'none';
        showToast(`Привет, ${playerName}!`);
        if (window.pendingScore) { saveScoreToLeaderboard(); window.pendingScore = false; }
    };
    ...
    const skipName = () => {
        ...
        localStorage.setItem('kolblocks_player_name', playerName);
        modal.style.display = 'none';
        showToast('Вы играете анонимно. Сменить имя можно в настройках.');
        if (window.pendingScore) { saveScoreToLeaderboard(); window.pendingScore = false; }
    };
    
    saveBtn.onclick = saveName;
    skipBtn.onclick = skipName;
    input.onkeypress = (e) => { if (e.key === 'Enter') saveName(); };
}

function changePlayerName() {
    localStorage.removeItem('kolblocks_player_name');
    playerName = '';
    showNameModal();
}

async function saveScoreToLeaderboard() {
    if (!window.firebaseDB) { console.warn('Firebase не инициализирован'); return false; }
    if (!playerName || playerName === '') { window.pendingScore = true; showNameModal(); return false; }
    
    try {
        const leaderboardRef = window.firebaseRef(window.firebaseDB, 'leaderboard');
        await window.firebasePush(leaderboardRef, {
            name: playerName,
            elo: Math.floor(playerELO),
            level: currentLevel - 1,
            combo: maxCombo,
            timestamp: Date.now()
        });
        showToast('🏆 Рекорд сохранён в таблицу лидеров!');
        return true;
    } catch (error) {
        console.error('Ошибка сохранения рекорда:', error);
        showToast('❌ Ошибка сохранения рекорда');
        return false;
    }
}

async function getLeaderboard() {
    if (!window.firebaseDB) { console.warn('Firebase не инициализирован'); return []; }
    try {
        const leaderboardRef = window.firebaseRef(window.firebaseDB, 'leaderboard');
        const topQuery = window.firebaseQuery(leaderboardRef, window.firebaseOrderByChild('elo'), window.firebaseLimitToLast(10));
        const snapshot = await window.firebaseGet(topQuery);
        const records = [];
        snapshot.forEach((childSnapshot) => { records.push({ id: childSnapshot.key, ...childSnapshot.val() }); });
        records.sort((a, b) => b.elo - a.elo);
        return records;
    } catch (error) { console.error('Ошибка получения таблицы рекордов:', error); return []; }
}

async function showLeaderboard() {
    const records = await getLeaderboard();
    let leaderboardDiv = document.getElementById('leaderboardContainer');
    if (!leaderboardDiv) {
        ...
        leaderboardDiv.style.background = 'rgba(0,0,0,0.5)';
        ...
        const buttons = pauseContent.querySelector('.pause-buttons');
        if (buttons) pauseContent.insertBefore(leaderboardDiv, buttons);
        else pauseContent.appendChild(leaderboardDiv);
    }
    
    if (records.length === 0) {
        leaderboardDiv.innerHTML = '<h3 style="color:#FFDE7D;">🏆 ТАБЛИЦА РЕКОРДОВ</h3><p style="color:#aaa;">Пока нет рекордов.<br>Станьте первым!</p>';
        return;
    }
    
    let html = '<h3 style="color:#FFDE7D; margin-bottom:15px;">🏆 ТОП-10 РЕКОРДОВ</h3>';
    html += '<div style="display:flex; justify-content:space-between; margin-bottom:10px; padding-bottom:5px; border-bottom:1px solid #444; color:#4af626; font-size:12px;"><span style="width:35px;">#</span><span style="flex:2;">ИГРОК</span><span style="width:60px;">ELO</span><span style="width:60px;">УРОВЕНЬ</span><span style="width:60px;">КОМБО</span></div>';
    
    records.forEach((record, index) => {
        const place = index + 1;
        let medal = `${place}.`;
        let bgColor = 'transparent';
        if (place === 1) { medal = '🥇'; bgColor = 'rgba(255,215,0,0.15)'; }
        else if (place === 2) { medal = '🥈'; bgColor = 'rgba(192,192,192,0.15)'; }
        else if (place === 3) { medal = '🥉'; bgColor = 'rgba(205,127,50,0.15)'; }
        
        html += `
            <div style="display:flex; justify-content:space-between; margin-bottom:8px; font-size:14px; padding:8px 5px; background:${bgColor}; border-radius:5px;">
                <span style="width:35px;">${medal}</span>
                <span style="flex:2; text-align:left; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${escapeHtml(record.name)}</span>
                <span style="width:60px; text-align:center; color:#FFDE7D; font-weight:bold;">${record.elo}</span>
                <span style="width:60px; text-align:center;">${record.level}</span>
                <span style="width:60px; text-align:center;">x${record.combo}</span>
            </div>
        `;
    });
...
class Player {
    ...
    meleeAttack() {
        ...
        if (equippedAura) {
            ...
            if (aura) { const screenX = centerX - cameraX; showAuraEffectOnPlayer(screenX, centerY, aura); }
        }
...
function loadBatidaoImage() {
    ...
    img.onerror = () => { console.log('Не удалось загрузить изображение'); batidaoImage = null; };
    img.src = 'https://images.genius.com/3849b06fe11fa1c89ba96465b298457c.1000x1000x1.png';
}

function checkAndShowNameModal() {
    if (!playerName || playerName === '') { showNameModal(); } 
    else { showToast(`Добро пожаловать, ${playerName}!`); }
}
