...
const bossTextures = {
    images: [], loaded: false,
    specialUrls: ['https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSbYIgWMe7urZ16o6fTz7OwE8IC6cakgU56rA&s', 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRg-WbZgPhjpjzrCi5jb0S3qJGJgSYway6EAQ&s', 'https://img07.rl0.ru/afisha/e1200x1200i/daily.afisha.ru/uploads/images/d/c5/dc57d49aef5e0a0bb334b47df0fff5c3.jpg', 'https://i.pinimg.com/736x/e2/ff/4a/e2ff4ab2a6496ba0632d23833569339d.jpg'],
    load() {
        return new Promise((resolve) => {
            let loadedCount = 0;
            const totalImages = this.specialUrls.length;
            this.specialUrls.forEach((url, index) => {
                const img = new Image();
                img.crossOrigin = "anonymous";
                img.onload = () => { loadedCount++; if (loadedCount === totalImages) { this.loaded = true; resolve(); } };
                img.onerror = () => { loadedCount++; if (loadedCount === totalImages) { this.loaded = true; resolve(); } };
                img.src = url;
                this.images[index] = img;
            });
        });
    },
    getRandomTexture(colors) {
        if (Math.random() < 0.7) return { type: 'color', color: colors[Math.floor(Math.random() * colors.length)] };
        else { const specialIndex = Math.floor(Math.random() * this.images.length); return { type: 'image', image: this.images[specialIndex] || null }; }
    }
};

// ==================== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ====================
function getClassData() { return PLAYER_CLASSES[currentClass] || PLAYER_CLASSES.warrior; }
function getCurrentSkinData() {
    if (equippedDecorated) return DECORATED_CUBES.find(s => s.id === equippedDecorated);
    if (equippedShape) return SHAPE_SKINS.find(s => s.id === equippedShape);
    return CHEST_SKINS.find(s => s.id === equippedSkin) || { id: 'default', name: 'Стандарт', color: '#4af626', shape: 'cube' };
}
...
function getSkinData(id) { const all = [{ id: 'default', name: 'Стандарт', color: '#4af626' }, ...CHEST_SKINS]; return all.find(s => s.id === id) || all[0]; }
function getAuraData(id) { if (!id) return null; return AURA_SKINS.find(a => a.id === id) || null; }
...
function renderShapes() {
    ...
    g.innerHTML = '';
    if (unlockedShapes.length === 0) { g.innerHTML = '<div style="color:#aaa; text-align:center; grid-column:1/-1;">Нет фигур. Открывайте кейс с фигурами за 15 🔑!</div>'; return; }
...
function renderDecorated() {
    ...
    g.innerHTML = '';
    if (unlockedDecorated.length === 0) { g.innerHTML = '<div style="color:#aaa; text-align:center; grid-column:1/-1;">Нет украшенных кубов. Открывайте кейс с украшениями за 20 🔑!</div>'; return; }
    DECORATED_CUBES.forEach(d => {
        const u = unlockedDecorated.includes(d.id), e = equippedDecorated === d.id;
        const el = document.createElement('div'); el.className = 'skin-item ' + (e ? 'equipped' : '') + (!u ? 'locked' : '');
...
function renderAuras() {
    ...
    g.innerHTML = '';
    if (unlockedAuras.length === 0) { g.innerHTML = '<div style="color:#aaa; text-align:center; grid-column:1/-1;">Нет аур. Открывайте кейс с аурой за 12 🔑!</div>'; return; }
...
function renderClasses() {
    ...
    Object.entries(PLAYER_CLASSES).forEach(([id, data]) => {
        ...
        const d = document.createElement('div'); d.className = 'class-item ' + (e ? 'equipped' : '') + (!u ? 'locked' : '');
...
function equipSkin(id) {
    if (unlockedSkins.includes(id)) { equippedSkin = id; equippedShape = null; equippedDecorated = null; saveAllData(); renderSkins(); renderShapes(); renderDecorated(); if (gameRunning && player) applyClassStats(); }
}
function equipShape(id) {
    if (unlockedShapes.includes(id)) { equippedShape = id; equippedSkin = 'default'; equippedDecorated = null; saveAllData(); renderSkins(); renderShapes(); renderDecorated(); if (gameRunning && player) applyClassStats(); }
}
function equipDecorated(id) {
    if (unlockedDecorated.includes(id)) { equippedDecorated = id; equippedSkin = 'default'; equippedShape = null; saveAllData(); renderSkins(); renderShapes(); renderDecorated(); if (gameRunning && player) applyClassStats(); }
}
...
function selectClass(id) {
    if (unlockedClasses.includes(id)) {
        ...
        } else { showResult('Не хватает ключей!', 'Нужно ' + classData.price + ' 🔑', '#ff2e63'); }
    }
}
...
function openChest() {
    ...
    setTimeout(() => {
        ...
        let roll = Math.random() * 100, cumulative = 0, drop = null;
        for (let s of CHEST_SKINS) { cumulative += s.chance; if (roll < cumulative) { drop = s; break; } }
        ...
        if (allSkinsUnlocked) { totalKeys += 10; saveAllData(); document.getElementById('shopKeyCount').textContent = totalKeys; showResult('🎁 Все скины собраны!', 'Возврат: +10 🔑', '#FFDE7D'); }
        else if (has) { const refund = 3; totalKeys += refund; saveAllData(); document.getElementById('shopKeyCount').textContent = totalKeys; showResult('🔄 Повторка: ' + drop.name, 'Возврат: +' + refund + ' 🔑', '#aaa'); }
        else { unlockedSkins.push(drop.id); saveAllData(); showResult('✨ Новый скин: ' + drop.name + '!', 'Получен ' + drop.name, drop.color); }
...
function openShapeChest() {
    ...
    setTimeout(() => {
        chest.classList.remove('shaking');
        let roll = Math.random() * 100, cumulative = 0, drop = null;
        for (let s of SHAPE_SKINS) { cumulative += s.chance; if (roll < cumulative) { drop = s; break; } }
        ...
        if (allShapesUnlocked) { totalKeys += 15; saveAllData(); document.getElementById('shopKeyCount').textContent = totalKeys; showResult('🎁 Все фигуры собраны!', 'Возврат: +15 🔑', '#FFDE7D'); }
        else if (has) { const refund = 5; totalKeys += refund; saveAllData(); document.getElementById('shopKeyCount').textContent = totalKeys; showResult('🔄 Повторка: ' + drop.name, 'Возврат: +' + refund + ' 🔑', '#aaa'); }
        else { unlockedShapes.push(drop.id); saveAllData(); showResult('✨ Новая фигура: ' + drop.name + '!', 'Теперь вы можете изменить форму!', drop.color); }
...
function openDecoratedChest() {
    ...
    setTimeout(() => {
        chest.classList.remove('shaking');
        let roll = Math.random() * 100, cumulative = 0, drop = null;
        for (let d of DECORATED_CUBES) { cumulative += d.chance; if (roll < cumulative) { drop = d; break; } }
        ...
        if (allDecoratedUnlocked) { totalKeys += 20; saveAllData(); document.getElementById('shopKeyCount').textContent = totalKeys; showResult('🎁 Все украшения собраны!', 'Возврат: +20 🔑', '#FFDE7D'); }
        else if (has) { const refund = 7; totalKeys += refund; saveAllData(); document.getElementById('shopKeyCount').textContent = totalKeys; showResult('🔄 Повторка: ' + drop.name, 'Возврат: +' + refund + ' 🔑', '#aaa'); }
        else { unlockedDecorated.push(drop.id); saveAllData(); showResult('✨ Новый куб: ' + drop.name + '!', 'Украшенный узором ' + drop.pattern, drop.color); }
...
function openAuraChest() {
    ...
    setTimeout(() => {
        chest.classList.remove('shaking');
        let roll = Math.random() * 100, cumulative = 0, drop = null;
        for (let a of AURA_SKINS) { cumulative += a.chance; if (roll < cumulative) { drop = a; break; } }
        ...
        if (allAurasUnlocked) { totalKeys += 12; saveAllData(); document.getElementById('shopKeyCount').textContent = totalKeys; showResult('🎁 Все ауры собраны!', 'Возврат: +12 🔑', '#FFDE7D'); }
        else if (has) { const refund = 4; totalKeys += refund; saveAllData(); document.getElementById('shopKeyCount').textContent = totalKeys; showResult('🔄 Повторка ауры: ' + drop.name, 'Возврат: +' + refund + ' 🔑', '#aaa'); }
        else { unlockedAuras.push(drop.id); saveAllData(); showResult('✨ Новая аура: ' + drop.name + '!', 'Теперь при ударе будет эффект!', drop.color); }
...
class Arrow {
    constructor(x, y, direction) {
        ...
        this.direction = direction; this.speed = 12; this.active = true; this.life = 120;
    }
    update() {
        this.x += this.speed * this.direction;
        this.life--;
        if (this.life <= 0 || this.x < cameraX - 100 || this.x > cameraX + canvas.width + 100) { this.active = false; return; }
        for (let i = 0; i < enemies.length; i++) {
            const e = enemies[i];
            if (!e.active) continue;
            if (this.x < e.x + e.width && this.x + this.width > e.x && this.y < e.y + e.height && this.y + this.height > e.y) {
                if (e.takeDamage(currentClass === 'archer' ? 2 : 1)) { enemies = enemies.filter(x => x !== e); }
                this.active = false; updateCombo();
        ...
        for (let i = 0; i < flyingEnemies.length; i++) {
            ...
            if (this.x < fe.x + fe.width && this.x + this.width > fe.x && this.y < fe.y + fe.height && this.y + this.height > fe.y) {
                if (fe.takeDamage(currentClass === 'archer' ? 2 : 1)) { flyingEnemies = flyingEnemies.filter(x => x !== fe); }
                this.active = false; updateCombo();
                for (let j = 0; j < 10; j++) particlePool.acquire(this.x, this.y, '#ffff00');
                break;
            }
        ...
        if (boss && boss.active) {
            if (this.x < boss.x + boss.width && this.x + this.width > boss.x && this.y < boss.y + boss.height && this.y + this.height > boss.y) {
                if (boss.takeDamage()) { boss = null; document.getElementById('bossHealthBar').style.display = 'none'; }
                this.active = false; updateCombo(); AudioSys.bossHit();
...
class Necromancer {
    constructor(x, y) {
        ...
        this.color = '#4a0e4e'; this.health = CONFIG.enemies.baseHealth.necromancer;
        this.maxHealth = this.health; this.scoreValue = CONFIG.enemies.baseScore.necromancer;
        this.active = true; this.resurrectCooldown = 0; this.resurrectingEnemy = null; this.resurrectTimer = 0;
    }
    update() {
        if (!this.active) return;
        ...
        for (let platform of platforms) {
            if (this.x < platform.x + platform.width && this.x + this.width > platform.x && this.y + this.height > platform.y && this.y + this.height < platform.y + 30) {
                this.y = platform.y - this.height; break;
            }
        }
    }
    ...
    draw(ctx, cameraX) {
        ...
        ctx.fillStyle = '#2a0a2e'; ctx.beginPath(); ctx.arc(drawX + this.width / 2, this.y + 15, 18, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = '#ff00ff'; ctx.beginPath(); ctx.arc(drawX + 12, this.y + 12, 4, 0, Math.PI * 2); ctx.arc(drawX + 28, this.y + 12, 4, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = '#ffffff'; ctx.beginPath(); ctx.arc(drawX + 10, this.y + 10, 2, 0, Math.PI * 2); ctx.arc(drawX + 26, this.y + 10, 2, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = '#8B4513'; ctx.fillRect(drawX + 35, this.y + 20, 5, 30);
        ctx.fillStyle = '#ff00ff'; ctx.beginPath(); ctx.arc(drawX + 37, this.y + 18, 6, 0, Math.PI * 2); ctx.fill();
        if (this.resurrectTimer > 0) {
            ctx.fillStyle = `rgba(255, 0, 255, ${Math.sin(Date.now() / 200) * 0.5 + 0.5})`;
            ctx.beginPath(); ctx.arc(drawX + this.width / 2, this.y - 15, 15, 0, Math.PI * 2); ctx.fill();
            const percent = this.resurrectTimer / 180;
            ctx.fillStyle = '#ff00ff'; ctx.fillRect(drawX + 10, this.y - 8, this.width * percent, 4);
        }
...
class ShooterEnemy {
    constructor(x, y) {
        ...
        this.color = '#ff6600'; this.health = 2; this.maxHealth = 2;
        this.active = true; this.shootCooldown = 0; this.fireballs = [];
    }
    update() {
        if (!this.active) return;
        this.shootCooldown--;
        if (this.shootCooldown <= 0 && player && Math.abs(player.x - this.x) < 400) {
            ...
            this.shootCooldown = 60; AudioSys.fireball();
        }
        for (let i = 0; i < this.fireballs.length; i++) {
            const fb = this.fireballs[i];
            fb.x += 6 * fb.direction;
            if (fb.x < cameraX - 100 || fb.x > cameraX + canvas.width + 100) { this.fireballs.splice(i, 1); i--; continue; }
            if (player && fb.x < player.x + player.width && fb.x + 10 > player.x && fb.y < player.y + player.height && fb.y + 10 > player.y) {
                player.takeDamage(15, fb.direction * 8, -5, '#ff6600'); this.fireballs.splice(i, 1); i--;
            }
        }
        this.y += 0.8;
        for (let platform of platforms) {
    ...
    takeDamage(amount = 1) { this.health -= amount; if (this.health <= 0) { this.destroy(); return true; } return false; }
    destroy() { this.active = false; addScore(150 * comboMultiplier); updateCombo(); for (let i = 0; i < 20; i++) particlePool.acquire(this.x + this.width / 2, this.y + this.height / 2, this.color); }
    draw(ctx, cameraX) {
        if (!this.active) return;
        const drawX = this.x - cameraX;
        ctx.fillStyle = this.color; ctx.fillRect(drawX, this.y, this.width, this.height);
        ctx.fillStyle = '#ff0000'; ctx.fillRect(drawX + 8, this.y + 8, 8, 8); ctx.fillRect(drawX + 24, this.y + 8, 8, 8);
        ctx.fillStyle = '#000'; ctx.fillRect(drawX + 10, this.y + 25, 20, 4);
        for (const fb of this.fireballs) {
            ctx.fillStyle = '#ff6600'; ctx.beginPath(); ctx.arc(fb.x - cameraX, fb.y, 8, 0, Math.PI * 2); ctx.fill();
            ctx.fillStyle = '#ffff00'; ctx.beginPath(); ctx.arc(fb.x - cameraX, fb.y, 4, 0, Math.PI * 2); ctx.fill();
        }
        if (this.health < this.maxHealth) { ctx.fillStyle = '#4af626'; ctx.fillRect(drawX, this.y - 8, (this.width * this.health) / this.maxHealth, 4); }
    }
}
class Vortex {
    constructor(x, y) {
        ...
        this.color = '#9b59b6'; this.health = CONFIG.enemies.baseHealth.vortex;
        this.maxHealth = this.health; this.scoreValue = CONFIG.enemies.baseScore.vortex;
        this.active = true; this.pullStrength = 3; this.rotation = 0;
    }
    update() {
        if (!this.active) return;
        this.rotation += 0.1;
        if (player) {
            ...
            if (dist < 200) {
                ...
                player.velX -= Math.cos(angle) * this.pullStrength;
                player.velY -= Math.sin(angle) * this.pullStrength;
            }
        }
        this.y += 0.8;
        for (let platform of platforms) {
    ...
    draw(ctx, cameraX) {
        ...
        ctx.restore();
        ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(drawX + 15, this.y + 18, 6, 0, Math.PI * 2); ctx.arc(drawX + 30, this.y + 18, 6, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = '#000';
        ...
        ctx.beginPath(); ctx.arc(drawX + 15 + Math.cos(angle1) * 3, this.y + 18 + Math.sin(angle1) * 3, 3, 0, Math.PI * 2); ctx.fill();
        ctx.beginPath(); ctx.arc(drawX + 30 + Math.cos(angle2) * 3, this.y + 18 + Math.sin(angle2) * 3, 3, 0, Math.PI * 2); ctx.fill();
        if (this.health < this.maxHealth) { ctx.fillStyle = '#4af626'; ctx.fillRect(drawX, this.y - 8, (this.width * this.health) / this.maxHealth, 4); }
    }
}
class Bum {
    constructor(x, y, platform) {
        ...
        this.active = true; this.platform = platform; this.fireballCooldown = 0; this.beardPhase = 0; this.fireballs = [];
    }
    update() {
        if (!this.active) return;
        this.beardPhase += 0.1; this.fireballCooldown--;
        if (this.platform) {
            ...
            if (this.platform.lavaDamageTimer >= 60 && player) {
                if (player.x < this.platform.x + this.platform.width && player.x + player.width > this.platform.x && player.y + player.height > this.platform.y && player.y < this.platform.y + this.platform.height) {
        ...
        if (this.fireballCooldown <= 0 && player && Math.abs(player.x - this.x) < 500) {
            const direction = player.x > this.x ? 1 : -1;
            this.fireballs.push({ x: this.x + this.width / 2, y: this.y + this.height / 2, direction: direction });
            this.fireballCooldown = 45; AudioSys.fireball();
        }
        for (let i = 0; i < this.fireballs.length; i++) {
            const fb = this.fireballs[i];
            fb.x += 5 * fb.direction;
            if (fb.x < cameraX - 100 || fb.x > cameraX + canvas.width + 100) { this.fireballs.splice(i, 1); i--; continue; }
            if (player && fb.x < player.x + player.width && fb.x + 12 > player.x && fb.y < player.y + player.height && fb.y + 12 > player.y) {
                player.takeDamage(20, fb.direction * 10, -5, '#ff4400'); this.fireballs.splice(i, 1); i--;
            }
        }
    }
    takeDamage(amount = 1) { this.health -= amount; if (this.health <= 0) { this.destroy(); return true; } return false; }
    destroy() {
        this.active = false; if (this.platform) this.platform.isLava = false;
        addScore(this.scoreValue * comboMultiplier); updateCombo(); AudioSys.collect();
    ...
    draw(ctx, cameraX) {
        ...
        const drawX = this.x - cameraX;
        ctx.fillStyle = this.color; ctx.fillRect(drawX, this.y, this.width, this.height);
        ctx.fillStyle = '#654321'; ctx.beginPath(); ctx.moveTo(drawX + 10, this.y + 30); ctx.quadraticCurveTo(drawX + 20, this.y + 45 + Math.sin(this.beardPhase) * 3, drawX + 30, this.y + 30); ctx.fill();
        ctx.fillStyle = '#fff'; ctx.fillRect(drawX + 8, this.y + 12, 8, 6); ctx.fillRect(drawX + 24, this.y + 12, 8, 6);
        ctx.fillStyle = '#ff0000'; ctx.fillRect(drawX + 9, this.y + 13, 6, 4); ctx.fillRect(drawX + 25, this.y + 13, 6, 4);
        ctx.fillStyle = '#2e8b57'; ctx.fillRect(drawX + 5, this.y - 8, 30, 10); ctx.fillRect(drawX + 12, this.y - 15, 16, 10);
        for (const fb of this.fireballs) {
            ctx.fillStyle = '#ff4400'; ctx.beginPath(); ctx.arc(fb.x - cameraX, fb.y, 10, 0, Math.PI * 2); ctx.fill();
            ctx.fillStyle = '#ffff00'; ctx.beginPath(); ctx.arc(fb.x - cameraX, fb.y, 5, 0, Math.PI * 2); ctx.fill();
        }
...
class Boss {
    constructor(x, y) {
        ...
        this.health = this.maxHealth; this.speed = CONFIG.boss.moveSpeed;
        this.direction = 1; this.attackCooldown = 0; this.active = true;
        this.texture = bossTextures.getRandomTexture(enemyColors);
        this.phase = 0; this.movePhase = 0; this.shootPattern = 0;
        this.color = this.texture.type === 'color' ? this.texture.color : '#ff0000';
    }
    update() {
        if (!this.active) return;
        this.movePhase += 0.02; this.attackCooldown--;
        if (Math.abs(this.x - player.x) > 200) { this.x += this.speed * this.direction; if (this.x < cameraX + 100 || this.x > cameraX + canvas.width - 100 - this.width) this.direction *= -1; }
        this.y += Math.sin(this.movePhase) * 2;
        if (this.attackCooldown <= 0) { this.performAttack(); this.attackCooldown = CONFIG.boss.attackCooldown - (currentLevel * 2); }
        if (player.checkCollision(this)) { player.takeDamage(CONFIG.boss.damage, this.x < player.x ? 15 : -15, -10, '#ff0000'); }
    }
    performAttack() {
        this.shootPattern = (this.shootPattern + 1) % 3;
        switch (this.shootPattern) {
            case 0: for (let i = 0; i < 15; i++) particlePool.acquire(this.x + this.width / 2, this.y + this.height / 2, '#ff0000'); break;
            case 1: for (let i = -1; i <= 1; i++) particlePool.acquire(this.x + this.width / 2, this.y + this.height / 2, '#ff6600'); break;
            case 2: for (let i = 0; i < 8; i++) { const angle = (i / 8) * Math.PI * 2; particlePool.acquire(this.x + this.width / 2, this.y + this.height / 2, '#ff0000'); } break;
        }
        for (let i = 0; i < 15; i++) particlePool.acquire(this.x + this.width / 2, this.y + this.height / 2, '#ff0000');
    }
    takeDamage(amount = 1) { this.health -= amount; AudioSys.bossHit(); for (let i = 0; i < 10; i++) particlePool.acquire(this.x + Math.random() * this.width, this.y + Math.random() * this.height, '#ff0000'); if (this.health <= 0) { this.defeat(); return true; } updateBossHealth(); return false; }
    defeat() {
        this.active = false; AudioSys.bossDefeat(); bossesDefeated++;
        for (let i = 0; i < 100; i++) particlePool.acquire(this.x + this.width / 2, this.y + this.height / 2, this.color);
        addScore(5000 * comboMultiplier); updateCombo(); document.getElementById('bossHealthBar').style.display = 'none';
    ...
    draw(ctx, cameraX) {
        ...
        if (this.texture.type === 'image' && this.texture.image) { try { ctx.drawImage(this.texture.image, drawX, this.y, this.width, this.height); } catch (e) { ctx.fillStyle = this.color; ctx.fillRect(drawX, this.y, this.width, this.height); } }
        else { ctx.fillStyle = this.color; ctx.fillRect(drawX, this.y, this.width, this.height); ctx.fillStyle = 'rgba(0,0,0,0.3)'; for (let i = 0; i < this.width; i += 20) for (let j = 0; j < this.height; j += 20) if ((i / 20 + j / 20) % 2 === 0) ctx.fillRect(drawX + i, this.y + j, 20, 20); }
        ctx.shadowBlur = 0;
        ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(drawX + 25, this.y + 30, 10, 0, Math.PI * 2); ctx.arc(drawX + 55, this.y + 30, 10, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = '#000'; ctx.beginPath(); ctx.arc(drawX + 25 + (this.direction * 3), this.y + 30, 5, 0, Math.PI * 2); ctx.arc(drawX + 55 + (this.direction * 3), this.y + 30, 5, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = '#000'; ctx.fillRect(drawX + 20, this.y + 55, 40, 10);
...
class Player {
    ...
    reset() {
        ...
        this.x = 100; this.y = 200; this.velX = 0; this.velY = 0;
        this.jumping = false; this.jumpCount = 0;
        this.maxJumps = classData.tripleJump ? 3 : 2;
        this.color = classData.color; this.speed = classData.speed; this.jumpPower = classData.jumpPower;
        ...
        this.dashCharges = classData.dashCharges; this.maxDashes = classData.dashCharges;
        this.dashCooldown = 0; this.isDashing = false; this.dashTimer = 0; this.dashDirection = 1;
    ...
    update(keys) {
        ...
        this.velY += currentClass === 'mage' ? this.gravity * 0.7 : this.gravity;
        ...
        if (currentClass === 'archer' && (keys['e'] || keys['E'] || keys['у'] || keys['У']) && this.arrowCooldown <= 0 && gameRunning) {
            arrows.push(new Arrow(this.x + this.width / 2, this.y + this.height / 2 - 10, this.dashDirection));
            this.arrowCooldown = 20; AudioSys.arrowShoot();
    ...
    startDash() {
        this.isDashing = true; this.dashTimer = CONFIG.player.dashDuration;
        this.dashCharges--; this.dashCooldown = CONFIG.player.dashCooldown;
    ...
    meleeAttack() {
        ...
        this.swingEffect = 8; AudioSys.meleeSwing();
        ...
        if (equippedAura) { const aura = getAuraData(equippedAura); if (aura) { showAuraEffectOnPlayer(centerX - cameraX, centerY, aura); } }
        let hitSomething = false;
        for (let i = 0; i < enemies.length; i++) { const e = enemies[i]; if (!e.active) continue; const dist = Math.hypot(centerX - (e.x + e.width / 2), centerY - (e.y + e.height / 2)); if (dist < radius) { for (let d = 0; d < damage; d++) { if (e.takeDamage(1)) { enemies = enemies.filter(x => x !== e); break; } } hitSomething = true; updateCombo(); this.createHitEffect(e.x + e.width / 2, e.y + e.height / 2); } }
        for (let i = 0; i < flyingEnemies.length; i++) { const fe = flyingEnemies[i]; if (!fe.active) continue; const dist = Math.hypot(centerX - (fe.x + fe.width / 2), centerY - (fe.y + fe.height / 2)); if (dist < radius) { for (let d = 0; d < damage; d++) { if (fe.takeDamage(1)) { flyingEnemies = flyingEnemies.filter(x => x !== fe); break; } } hitSomething = true; updateCombo(); this.createHitEffect(fe.x + fe.width / 2, fe.y + fe.height / 2); } }
        if (boss && boss.active) { const dist = Math.hypot(centerX - (boss.x + boss.width / 2), centerY - (boss.y + boss.height / 2)); if (dist < radius + 15) { for (let d = 0; d < damage; d++) { if (boss.takeDamage()) { boss = null; document.getElementById('bossHealthBar').style.display = 'none'; break; } } hitSomething = true; updateCombo(); this.createHitEffect(boss.x + boss.width / 2, boss.y + boss.height / 2); AudioSys.bossHit(); } }
        if (hitSomething) { for (let i = 0; i < 25; i++) particlePool.acquire(centerX + (Math.random() - 0.5) * 40, centerY + (Math.random() - 0.5) * 40, '#ffaa33'); shakeScreen(3); }
    }
    ...
    takeDamage(amount, knockbackX, knockbackY, color) {
        ...
        playerHealth = Math.max(0, playerHealth - amount); updateHealthBar();
        this.velX = knockbackX; this.velY = knockbackY; this.jumping = true;
        this.invulnerable = 45; AudioSys.hit(); roundDamage += amount;
    ...
    draw(ctx, cameraX) {
        ...
        const drawX = this.x - cameraX;
        for (let i = 0; i < this.trail.length; i++) { ctx.globalAlpha = i / this.trail.length * 0.25; ctx.fillStyle = this.isDashing ? '#FFDE7D' : skinData.color; if (shape === 'sphere') { ctx.beginPath(); ctx.arc(this.trail[i].x - cameraX + this.width / 2, this.trail[i].y + this.height / 2, this.width / 2, 0, Math.PI * 2); ctx.fill(); } else { ctx.fillRect(this.trail[i].x - cameraX, this.trail[i].y, this.width, this.height); } }
        ctx.globalAlpha = 1;
        if (this.isDashing) ctx.shadowColor = skinData.color;
        if (this.isDashing) ctx.shadowBlur = 20;
        if (shape === 'sphere') {
            ctx.fillStyle = skinData.color; ctx.beginPath(); ctx.arc(drawX + this.width / 2, this.y + this.height / 2, this.width / 2, 0, Math.PI * 2); ctx.fill();
            if (skinData.eyes) {
                ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(drawX + 12, this.y + 15, 5, 0, Math.PI * 2); ctx.arc(drawX + 28, this.y + 15, 5, 0, Math.PI * 2); ctx.fill();
                ctx.fillStyle = '#000'; ctx.beginPath(); ctx.arc(drawX + 11 + (this.velX * 0.5), this.y + 14, 3, 0, Math.PI * 2); ctx.arc(drawX + 27 + (this.velX * 0.5), this.y + 14, 3, 0, Math.PI * 2); ctx.fill();
                const moveX = Math.min(Math.max(this.velX * 0.3, -2), 2);
                ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(drawX + 10 + moveX, this.y + 13, 1.5, 0, Math.PI * 2); ctx.arc(drawX + 26 + moveX, this.y + 13, 1.5, 0, Math.PI * 2); ctx.fill();
            }
        } else if (shape === 'diamond') {
            ctx.fillStyle = skinData.color; ctx.beginPath(); ctx.moveTo(drawX + this.width / 2, this.y); ctx.lineTo(drawX + this.width, this.y + this.height / 2); ctx.lineTo(drawX + this.width / 2, this.y + this.height); ctx.lineTo(drawX, this.y + this.height / 2); ctx.closePath(); ctx.fill();
        } else {
            ctx.fillStyle = skinData.color; ctx.fillRect(drawX, this.y, this.width, this.height);
            if (skinData.pattern === 'crown') { ctx.fillStyle = '#FFD700'; ctx.fillRect(drawX + 10, this.y + 5, 6, 6); ctx.fillRect(drawX + 24, this.y + 5, 6, 6); ctx.fillRect(drawX + 17, this.y + 2, 6, 6); }
            else if (skinData.pattern === 'runes') { ctx.fillStyle = '#fff'; ctx.font = 'bold 12px monospace'; ctx.fillText('⚡', drawX + 15, this.y + 25); ctx.fillText('🌀', drawX + 28, this.y + 25); }
            else if (skinData.pattern === 'gears') { ctx.fillStyle = '#aaa'; ctx.beginPath(); ctx.arc(drawX + 12, this.y + 20, 6, 0, Math.PI * 2); ctx.fill(); ctx.beginPath(); ctx.arc(drawX + 28, this.y + 20, 6, 0, Math.PI * 2); ctx.fill(); }
            else if (skinData.pattern === 'stars') { ctx.fillStyle = '#fff'; for (let i = 0; i < 5; i++) { ctx.fillRect(drawX + 5 + i * 7, this.y + 8, 2, 2); ctx.fillRect(drawX + 5 + i * 7, this.y + 30, 2, 2); } }
            else if (skinData.pattern === 'grid') { ctx.strokeStyle = '#0ff'; ctx.lineWidth = 1; for (let i = 0; i < 4; i++) { ctx.beginPath(); ctx.moveTo(drawX + i * 10, this.y); ctx.lineTo(drawX + i * 10, this.y + this.height); ctx.stroke(); ctx.beginPath(); ctx.moveTo(drawX, this.y + i * 10); ctx.lineTo(drawX + this.width, this.y + i * 10); ctx.stroke(); } }
            else if (skinData.pattern === 'skull') { ctx.fillStyle = '#000'; ctx.fillRect(drawX + 10, this.y + 8, 6, 6); ctx.fillRect(drawX + 24, this.y + 8, 6, 6); ctx.fillRect(drawX + 15, this.y + 28, 10, 4); }
        }
        ctx.fillStyle = '#222';
        if (shape !== 'sphere') { ctx.fillRect(drawX + 10, this.y + 10, 8, 8); ctx.fillRect(drawX + 22, this.y + 10, 8, 8); ctx.fillRect(drawX + 10, this.y + 25, 20, 4); }
        ctx.shadowBlur = 0;
        ctx.strokeStyle = this.invulnerable > 0 && Math.floor(this.invulnerable / 4) % 2 === 0 ? '#fff' : skinData.color;
        ctx.lineWidth = 2;
        if (shape === 'sphere') { ctx.beginPath(); ctx.arc(drawX + this.width / 2, this.y + this.height / 2, this.width / 2, 0, Math.PI * 2); ctx.stroke(); }
        else { ctx.strokeRect(drawX, this.y, this.width, this.height); }
        if (this.swingEffect > 0) { const radius = getClassData().meleeRadius || CONFIG.melee.radius; ctx.beginPath(); ctx.arc(drawX + this.width / 2, this.y + this.height / 2, radius, 0, Math.PI * 2); ctx.fillStyle = `rgba(255, 80, 40, ${0.3 * (this.swingEffect / 8)})`; ctx.fill(); ctx.beginPath(); ctx.arc(drawX + this.width / 2, this.y + this.height / 2, radius - 10, 0, Math.PI * 2); ctx.fillStyle = `rgba(255, 200, 0, 0.5)`; ctx.fill(); }
        ...
        if (this.tripleJump && this.jumpCount === 2 && !this.jumping) { ctx.fillStyle = 'rgba(255, 170, 0, 0.7)'; ctx.beginPath(); ctx.arc(drawX + this.width / 2, this.y - 8, 5, 0, Math.PI * 2); ctx.fill(); }
    }
}

// ==================== ГЕНЕРАЦИЯ УРОВНЯ ====================
function generateLevel(level) {
    platforms = []; enemies = []; flyingEnemies = []; coins = []; powerUps = []; levelKeys = []; arrows = []; necromancerResurrectQueue = [];
    ...
    for (let i = 0; i < platformCount; i++) {
        ...
        if (i === 4 && level % 3 === 0) bumPlatform = newPlatform;
    ...
    if (isBossLevel) {
        for (let i = 0; i < Math.floor(enemyCount / 2); i++) { const pi = Math.floor(Math.random() * (platforms.length - 5)) + 2; const p = platforms[pi]; const enemyType = Math.floor(Math.random() * 4); if (enemyType === 0) enemies.push(new Enemy(p.x + p.width / 2 - 20, p.y - 40, 0)); else if (enemyType === 1) enemies.push(new Enemy(p.x + p.width / 2 - 20, p.y - 40, 1)); else if (enemyType === 2) enemies.push(new Enemy(p.x + p.width / 2 - 20, p.y - 40, 2)); else enemies.push(new Necromancer(p.x + p.width / 2 - 20, p.y - 50)); }
        ...
        for (let i = 0; i < enemyCount; i++) { const pi = Math.floor(Math.random() * (platforms.length - 5)) + 2; const p = platforms[pi]; const enemyRoll = Math.random(); if (enemyRoll < 0.4) enemies.push(new Enemy(p.x + p.width / 2 - 20, p.y - 40, Math.floor(Math.random() * 3))); else if (enemyRoll < 0.6 && level > 2) enemies.push(new Vortex(p.x + p.width / 2 - 20, p.y - 45)); else if (enemyRoll < 0.75 && level > 3) enemies.push(new Necromancer(p.x + p.width / 2 - 20, p.y - 50)); else enemies.push(new Enemy(p.x + p.width / 2 - 20, p.y - 40, Math.floor(Math.random() * 3))); }
        if (level % 3 === 0 && bumPlatform && level > 1) { const bumEnemy = new Bum(bumPlatform.x + bumPlatform.width / 2 - 20, bumPlatform.y - 45, bumPlatform); enemies.push(bumEnemy); }
...
function gameLoop() {
    ...
    for (let e of flyingEnemies) { e.update(); e.draw(ctx); }
...
function completeLevel() {
    gameRunning = false; if (gameLoopId) cancelAnimationFrame(gameLoopId);
    AudioSys.levelComplete(); addScore(1000 * currentLevel);
    ...
    setTimeout(() => {
        ...
        player = new Player(); applyClassStats();
        playerHealth = maxHealth; updateHealthBar();
        cameraX = 0; comboCount = 1; comboMultiplier = 1; maxCombo = 1; lastCheckpointX = 0;
        updateUI(); updateDashIndicator(); updateEloDisplay();
        gameRunning = true; gameLoop();
    }, 2500);
}
function gameOver() { gameRunning = false; if (gameLoopId) cancelAnimationFrame(gameLoopId); AudioSys.gameOver(); document.getElementById('finalScore').textContent = score; document.getElementById('finalLevel').textContent = currentLevel - 1; document.getElementById('finalCombo').textContent = `x${maxCombo}`; document.getElementById('bossesDefeated').textContent = bossesDefeated; document.getElementById('finalElo').textContent = Math.floor(playerELO); document.getElementById('gameOver').style.display = 'flex'; }
function restartGame() { if (gameLoopId) cancelAnimationFrame(gameLoopId); document.getElementById('gameOver').style.display = 'none'; document.getElementById('pauseMenu').style.display = 'none'; document.getElementById('caseShopScreen').style.display = 'none'; currentLevel = 1; score = 0; playerHealth = maxHealth; comboCount = 1; maxCombo = 1; comboMultiplier = 1; lastCheckpointX = 0; bossesDefeated = 0; roundCoins = 0; roundDamage = 0; generateLevel(currentLevel); player = new Player(); applyClassStats(); cameraX = 0; updateUI(); updateHealthBar(); updateDashIndicator(); updateEloDisplay(); document.getElementById('bossHealthBar').style.display = 'none'; gameRunning = true; gameLoop(); }
function togglePause() { const pm = document.getElementById('pauseMenu'); if (pm.style.display === 'flex') { pm.style.display = 'none'; gameRunning = true; } else { showPauseMenu(); } }
function showPauseMenu() { gameRunning = false; if (gameLoopId) cancelAnimationFrame(gameLoopId); document.getElementById('pauseMenu').style.display = 'flex'; }
...
function showAuraEffectOnPlayer(x, y, aura) {
    ...
    effect.style.position = 'fixed'; effect.style.pointerEvents = 'none'; effect.style.zIndex = '200'; effect.style.borderRadius = '50%';
    if (aura.id === 'batidao_aura' && batidaoImage) {
        effect.style.background = `radial-gradient(circle, ${aura.effectColor} 0%, transparent 70%)`;
        effect.style.backgroundImage = `url(${batidaoImage.src})`; effect.style.backgroundSize = 'cover'; effect.style.backgroundPosition = 'center'; effect.style.backgroundBlend = 'overlay';
        ...
        for (let i = 0; i < 30; i++) { setTimeout(() => { const particle = document.createElement('div'); particle.style.position = 'fixed'; particle.style.left = (x + (Math.random() - 0.5) * 180) + 'px'; particle.style.top = (y + (Math.random() - 0.5) * 180) + 'px'; particle.style.width = (Math.random() * 8 + 4) + 'px'; particle.style.height = (Math.random() * 8 + 4) + 'px'; particle.style.background = `#ff${Math.floor(Math.random() * 55 + 200).toString(16)}00`; particle.style.borderRadius = '50%'; particle.style.boxShadow = `0 0 15px ${aura.color}`; particle.style.pointerEvents = 'none'; particle.style.zIndex = '199'; particle.style.animation = 'particleExplode 0.5s ease-out forwards'; document.body.appendChild(particle); setTimeout(() => particle.remove(), 500); }, i * 10); }
    } else { effect.style.background = `radial-gradient(circle, ${aura.effectColor}, transparent)`; effect.style.boxShadow = `0 0 40px ${aura.color}`; effect.style.animation = 'auraExpandPlayer 0.4s ease-out forwards'; }
    effect.style.left = (x - 150) + 'px'; effect.style.top = (y - 150) + 'px'; effect.style.width = '300px'; effect.style.height = '300px';
    document.body.appendChild(effect); activeAuraEffect = effect;
    setTimeout(() => { if (activeAuraEffect) { activeAuraEffect.remove(); activeAuraEffect = null; } }, 500);
}
const styleSheet = document.createElement("style");
styleSheet.textContent = `@keyframes auraExpandPlayer { 0% { transform: scale(0.5) translate(0, 0); opacity: 0.8; } 100% { transform: scale(1.5) translate(0, 0); opacity: 0; } } @keyframes batidaoAuraPlayer { 0% { transform: scale(0.3) rotate(0deg); opacity: 1; } 50% { transform: scale(1.2) rotate(180deg); opacity: 0.8; } 100% { transform: scale(1.8) rotate(360deg); opacity: 0; } } @keyframes particleExplode { 0% { transform: scale(1); opacity: 1; } 100% { transform: scale(0) translateY(-60px); opacity: 0; } } .player-aura { transform-origin: center center; }`;
...
function setupMobileControls() {
    ...
    if (isTouch || window.innerWidth <= 768) {
        ...
        const bind = (id, key) => { const btn = document.getElementById(id); btn.addEventListener('touchstart', (e) => { e.preventDefault(); keys[key] = true; }); btn.addEventListener('touchend', (e) => { e.preventDefault(); keys[key] = false; }); btn.addEventListener('mousedown', () => keys[key] = true); btn.addEventListener('mouseup', () => keys[key] = false); btn.addEventListener('mouseleave', () => keys[key] = false); };
...
async function initGame() {
    ...
    let progress = 0;
    const pi = setInterval(() => { progress += Math.random() * 15; document.getElementById('progressFill').style.width = `${Math.min(progress, 100)}%`; if (progress >= 100) { clearInterval(pi); setTimeout(() => { document.getElementById('loading').style.opacity = '0'; setTimeout(() => document.getElementById('loading').style.display = 'none', 500); }, 300); } }, 100);
    generateLevel(currentLevel);
    player = new Player();
    applyClassStats();
    updateUI();
