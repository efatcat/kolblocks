...
let arrows = []; // Массив для стрел лучника
...
function renderShapes() {
    const g = document.getElementById('shapesGrid'); g.innerHTML = '';
...
function renderDecorated() {
    const g = document.getElementById('decoratedGrid'); g.innerHTML = '';
...
class Arrow {
    ...
    update() {
        ...
        // Проверка попадания в летающих врагов
...
// Некромант - может воскрешать врагов
class Necromancer {
    constructor(x, y) {
        this.x = x;
        this.y = y;
    ...
    update() {
        ...
        // Поиск мертвого врага для воскрешения
        if (this.resurrectCooldown <= 0 && !this.resurrectingEnemy) {
            // Ищем недавно убитого врага (хранится в necromancerResurrectQueue)
            for (let i = 0; i < necromancerResurrectQueue.length; i++) {
                ...
                if (deadEnemy && Math.abs(deadEnemy.x - this.x) < 300) {
                    ...
                    this.resurrectTimer = 180; // 3 секунды при 60 FPS
        ...
        if (this.resurrectingEnemy && this.resurrectTimer > 0) {
            ...
            if (this.resurrectTimer <= 0) {
                // Воскрешаем врага как стреляющего
...
// Стреляющий враг (для воскрешения некромантом)
class ShooterEnemy {
    constructor(x, y) {
        this.x = x;
        this.y = y;
...
class Bum {
    constructor(x, y, platform) {
        ...
        this.height = 45;
        this.color = '#8B4513';
        this.health = 5; // 5 ударов
        this.maxHealth = 5;
        this.scoreValue = CONFIG.enemies.baseScore.bum;
        this.active = true;
        this.platform = platform; // Платформа, на которой стоит
    ...
    update() {
        ...
        if (this.fireballCooldown <= 0 && player && Math.abs(player.x - this.x) < 500) {
            const direction = player.x > this.x ? 1 : -1;
            const fireball = { x: this.x + this.width/2, y: this.y + this.height/2, direction: direction, active: true };
...
class Player {
    ...
    update(keys) {
        ...
        // Прыжки (двойной или тройной для разбойника)
        const maxJumps = this.tripleJump ? 3 : (CONFIG.player.doubleJump ? 2 : 1);
...
// Остальные классы (Boss, Enemy, FlyingEnemy, Platform) остаются без изменений, 
// но с добавлением новых врагов в генерацию уровня

// ==================== ГЕНЕРАЦИЯ УРОВНЯ (с новыми врагами) ====================
function generateLevel(level){ 
    ...
    for(let i=0;i<platformCount;i++){ 
        ...
        // Запоминаем платформу для бомжа на 4 уровне
        if (i === 4 && level % 3 === 0) {
    ...
    if(isBossLevel){ 
        for(let i=0;i<Math.floor(enemyCount/2);i++){const pi=Math.floor(Math.random()*(platforms.length-5))+2;const p=platforms[pi];
            // Разнообразие врагов
        ...
        for(let i=0;i<enemyCount;i++){const pi=Math.floor(Math.random()*(platforms.length-5))+2;const p=platforms[pi];
        ...
        // Бомж на специальной платформе (каждый 3 уровень)
...
window.addEventListener('resize', () => { resizeCanvas(); if(gameRunning){ generateLevel(currentLevel); player.reset(); applyClassStats(); cameraX = 0; } });
