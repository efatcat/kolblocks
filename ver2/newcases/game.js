...
const CHEST_SKINS = [
    ...
    { id: 'cube_decorated', name: 'Узорчатый куб', color: '#e74c3c', chance: 45, type: 'decorated', pattern: 'stripes' },
    { id: 'cube_marble', name: 'Мраморный куб', color: '#bdc3c7', chance: 30, type: 'decorated', pattern: 'marble' },
    { id: 'cube_neon', name: 'Неоновый призм', color: '#00ff88', chance: 25, type: 'decorated', pattern: 'neon' },
...
const CHEST_TYPES = [
    ...
    { id: 'decorated', name: 'Узорчатый кейс', price: 30, icon: '✨', skins: CHEST_SKINS.filter(s => ['cube_decorated', 'cube_marble', 'cube_neon'].includes(s.id)) },
...
let unlockedAuras = [];
let unlockedClasses = ['default'];
let equippedSkin = 'default';
let equippedAura = null;
let equippedClass = 'default';
let roundCoins = 0;
let roundDamage = 0;
...
playerELO = safeGetNum('kolblocks_elo', 0);
totalKeys = safeGetNum('kolblocks_keys', 0);
unlockedSkins = safeGet('kolblocks_skins', ['default']);
unlockedAuras = safeGet('kolblocks_auras', []);
unlockedClasses = safeGet('kolblocks_classes', ['default']);
equippedSkin = localStorage.getItem('kolblocks_equipped') || 'default';
equippedAura = localStorage.getItem('kolblocks_equipped_aura') || null;
equippedClass = localStorage.getItem('kolblocks_equipped_class') || 'default';
...
const AudioSys = {
    ...
    homelessShoot() { this.play(200, 0.2, 'sawtooth', 0.15, 0.3); }
};

// Разблокировка аудио по первому взаимодействию
document.addEventListener('click', () => AudioSys.resume(), { once: true });
document.addEventListener('touchstart', () => AudioSys.resume(), { once: true });
...
class Particle {
    ...
    draw(ctx, cx) {
        ...
        ctx.beginPath();
        ctx.arc(this.x - cx, this.y, this.size, 0, Math.PI * 2);
        ctx.fill();
        ctx.globalAlpha = 1;
    }
}
...
class Arrow {
    constructor(x, y, dir, dmg = 1) {
        this.x = x; this.y = y;
        this.w = 20; this.h = 4;
        this.spd = 15; this.dir = dir;
        this.dmg = dmg; this.active = true;
        this.life = 120; this.color = '#27ae60';
    }
    update() {
        if (!this.active) return;
        this.x += this.spd * this.dir;
        this.life--;
        if (this.life <= 0 || this.x < cameraX - 50 || this.x > cameraX + canvas.width + 50) {
    ...
    draw(ctx) {
        ...
        ctx.fillStyle = '#1a5233'; ctx.beginPath(); ctx.moveTo(this.w / 2, 0); ctx.lineTo(this.w / 2 + 8, -6); ctx.lineTo(this.w / 2 + 8, 6); ctx.fill();
        ctx.restore();
    }
}
...
class WindProjectile {
    constructor(x, y, targetX, targetY) {
        ...
        const dx = targetX - x, dy = targetY - y, dist = Math.sqrt(dx * dx + dy * dy) || 1;
        this.vx = (dx / dist) * this.speed; this.vy = (dy / dist) * this.speed; this.rotation = 0;
    }
    update() {
        if (!this.active) return;
        this.x += this.vx; this.y += this.vy; this.rotation += 0.2; this.life--;
        if (this.life <= 0 || this.x < cameraX - 50 || this.x > cameraX + canvas.width + 50 || this.y < -50 || this.y > canvas.height + 50) { this.active = false; return; }
        if (this.x < player.x + player.width && this.x + this.w > player.x && this.y < player.y + player.height && this.y + this.h > player.y) {
            player.takeDamage(12, this.vx * 0.5, this.vy * 0.3, '#00ffff'); this.active = false; AudioSys.windHit();
            for (let i = 0; i < 10; i++) particlePool.acquire(this.x, this.y, '#00ffff'); return;
        }
    ...
    draw(ctx) {
        ...
        ctx.fillStyle = 'rgba(0, 255, 255, 0.8)'; ctx.beginPath(); ctx.arc(0, 0, this.w / 2, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = 'rgba(255, 255, 255, 0.6)'; ctx.beginPath(); ctx.arc(-2, -2, 3, 0, Math.PI * 2); ctx.fill();
        ctx.restore();
    }
}

class LavaProjectile {
    constructor(x, y, angle) {
        this.x = x; this.y = y; this.w = 10; this.h = 10;
        this.speed = 6; this.active = true; this.life = 180; this.color = '#ff4500';
        this.vx = Math.cos(angle) * this.speed; this.vy = Math.sin(angle) * this.speed;
        this.gravity = 0.3; this.rotation = 0;
    }
    update() {
        if (!this.active) return;
        this.x += this.vx; this.y += this.vy; this.vy += this.gravity; this.rotation += 0.15; this.life--;
        if (this.life <= 0 || this.x < cameraX - 50 || this.x > cameraX + canvas.width + 50 || this.y > canvas.height + 50) { this.active = false; return; }
        if (this.x < player.x + player.width && this.x + this.w > player.x && this.y < player.y + player.height && this.y + this.h > player.y) {
            player.takeDamage(15, this.vx * 0.3, -8, '#ff4500'); this.active = false; AudioSys.lavaBurn();
            for (let i = 0; i < 12; i++) particlePool.acquire(this.x, this.y, '#ff4500'); return;
        }
        for (let p of platforms) {
            if (this.x < p.x + p.width && this.x + this.w > p.x && this.y < p.y + p.height && this.y + this.h > p.y) {
                this.active = false; for (let i = 0; i < 8; i++) particlePool.acquire(this.x, this.y, '#ff4500'); return;
            }
        }
    }
    draw(ctx) {
        if (!this.active) return;
        ctx.save(); ctx.translate(this.x - cameraX + this.w/2, this.y + this.h/2); ctx.rotate(this.rotation);
        ctx.fillStyle = '#ff6600'; ctx.beginPath(); ctx.arc(0, 0, this.w/2, 0, Math.PI*2); ctx.fill();
        ctx.fillStyle = '#ffcc00'; ctx.beginPath(); ctx.arc(-2, -2, 3, 0, Math.PI*2); ctx.fill();
        ctx.restore();
    }
}

// ==================== ТЕКСТУРЫ БОССОВ (БЕЗОПАСНАЯ ЗАГРУЗКА) ====================
const bossTextures = {
    images: [], loaded: false,
    urls: [
    ...
    load() {
        return new Promise(res => {
            ...
            if (total === 0) { this.loaded = true; res(); return; }
            this.urls.forEach((u, i) => {
                const img = new Image();
                img.onload = img.onerror = () => {
                    if (img.complete && img.naturalWidth > 0) this.images[i] = img;
                    else this.images[i] = null;
                    loaded++;
                    if (loaded >= total) { this.loaded = true; res(); }
                };
                img.src = u;
            });
            setTimeout(() => { this.loaded = true; res(); }, 3000);
        });
    },
    getRandom(colors) {
        const validImgs = this.images.filter(img => img !== null);
        if (validImgs.length > 0 && Math.random() < 0.6) {
            return { type: 'image', image: validImgs[Math.floor(Math.random() * validImgs.length)] };
        }
        return { type: 'color', color: colors[Math.floor(Math.random() * colors.length)] };
    }
};
...
const platTex = [{ c: '#FF2E63', p: 'stripes' }, { c: '#08D9D6', p: 'dots' }, { c: '#FFDE7D', p: 'checker' }, { c: '#6A2C70', p: 'zigzag' }, { c: '#4ECDC4', p: 'bricks' }, { c: '#FF9A76', p: 'waves' }];
...
function updateHealthBar() { const el = document.getElementById('healthFill'); if(el) el.style.width = `${(playerHealth / maxHealth) * 100}%`; }
function updateBossHealth() { const el = document.getElementById('bossHealthFill'); if(boss && el) el.style.width = `${(boss.health / boss.maxHealth) * 100}%`; }
function updateUI() {
    const lvl = document.getElementById('levelDisplay'); if(lvl) lvl.textContent = currentLevel;
    const sc = document.getElementById('scoreDisplay'); if(sc) sc.textContent = score;
    const cmb = document.getElementById('comboCount'); if(cmb) cmb.textContent = `x${comboCount}`;
    const elo = document.getElementById('eloValue'); if(elo) elo.textContent = Math.floor(playerELO);
    const cls = document.getElementById('classDisplay'); if(cls) cls.textContent = 'Класс: ' + PLAYER_CLASSES.find(c => c.id === equippedClass).name;
}
...
function updateCombo() {
    ...
    if(cd) { cd.classList.add('active'); const v = document.getElementById('comboValue'); if(v) v.textContent = comboCount; setTimeout(() => cd.classList.remove('active'), 1000); }
...
function showCheckpointIndicator() { const ci = document.getElementById('checkpointIndicator'); if(ci) { ci.classList.add('active'); setTimeout(() => ci.classList.remove('active'), 2000); } }
...
function equipClass(id) { if (unlockedClasses.includes(id)) { equippedClass = id; saveAllData(); applyClassStats(); renderClasses(); updateHealthBar(); updateUI(); } }
...
// ==================== СИСТЕМА ELO ====================
function updateEloDisplay() { const el = document.getElementById('eloValue'); if(el) el.textContent = Math.floor(playerELO); }
...
function calculateEloChange() {
    const c = Math.floor(roundCoins * CONFIG.elo.coinsMultiplier);
    const d = Math.floor(roundDamage / CONFIG.elo.damageDivider);
    const ch = c - d;
...
function saveAllData() {
    safeSet('kolblocks_elo', playerELO); safeSet('kolblocks_keys', totalKeys);
    safeSet('kolblocks_skins', unlockedSkins); safeSet('kolblocks_auras', unlockedAuras);
    safeSet('kolblocks_classes', unlockedClasses); safeSet('kolblocks_equipped', equippedSkin);
    safeSet('kolblocks_equipped_aura', equippedAura || ''); safeSet('kolblocks_equipped_class', equippedClass);
}

function openShop() {
    gameRunning = false; const pm = document.getElementById('pauseMenu'); if(pm) pm.style.display = 'none';
    const cs = document.getElementById('caseShopScreen'); if(cs) cs.style.display = 'flex';
    const sk = document.getElementById('shopKeyCount'); if(sk) sk.textContent = totalKeys;
    renderChestTypes(); renderSkins(); renderAuras(); renderClasses();
}
function closeShop() { const cs = document.getElementById('caseShopScreen'); if(cs) cs.style.display = 'none'; gameRunning = true; if (typeof gameLoop === 'function') gameLoop(); }
function renderChestTypes() {
    const c = document.getElementById('chestsContainer'); if(!c) return; c.innerHTML = '';
...
function openChestType(id, btn) {
    ...
    btn.disabled = true; btn.textContent = '...'; AudioSys.chestOpen();
    if (ch.isAura) {
        totalKeys -= ch.price; saveAllData(); const sk = document.getElementById('shopKeyCount'); if(sk) sk.textContent = totalKeys;
        setTimeout(() => {
            ...
            if (has) { totalKeys += 3; saveAllData(); const sk2 = document.getElementById('shopKeyCount'); if(sk2) sk2.textContent = totalKeys; showResult('Дубликат ауры: ' + dr.name, 'Возврат: +3 🔑', dr.color); }
    ...
    setTimeout(() => {
        ...
        if (has) { totalKeys += Math.floor(ch.price * 0.2); saveAllData(); const sk2 = document.getElementById('shopKeyCount'); if(sk2) sk2.textContent = totalKeys; showResult('Дубликат: ' + dr.name, 'Возврат: +' + Math.floor(ch.price * 0.2) + ' 🔑', dr.color); }
        else { unlockedSkins.push(dr.id); saveAllData(); showResult('Новый скин: ' + dr.name + '!', dr.color); }
        renderSkins(); renderChestTypes();
    }, 1200);
}
function renderSkins() {
    const g = document.getElementById('skinsGrid'); if(!g) return; g.innerHTML = '';
...
function renderAuras() {
    const g = document.getElementById('aurasGrid'); if(!g) return; g.innerHTML = '';
...
function renderClasses() {
    const g = document.getElementById('classesGrid'); if(!g) return; g.innerHTML = '';
...
function buyClass(id, price) { if (totalKeys < price) return; if (confirm(`Купить ${getClassData(id).name} за ${price} 🔑?`)) { totalKeys -= price; unlockedClasses.push(id); equippedClass = id; saveAllData(); applyClassStats(); renderClasses(); const sk = document.getElementById('shopKeyCount'); if(sk) sk.textContent = totalKeys; showResult('Класс получен!', getClassData(id).name, getClassData(id).color); } }
...
function openAuraChest() { if (totalKeys < 12) { alert('Нужно минимум 12 🔑!'); return; } totalKeys -= 12; saveAllData(); const sk = document.getElementById('shopKeyCount'); if(sk) sk.textContent = totalKeys; const btn = document.getElementById('openAuraChestBtn'), chest = document.getElementById('auraChestVisual'); if (btn) { btn.disabled = true; chest.classList.add('shaking'); } AudioSys.chestOpen(); setTimeout(() => { chest.classList.remove('shaking'); let r = Math.random() * 100, cm = 0, dr = AURA_SKINS[0]; for (let a of AURA_SKINS) { cm += a.chance; if (r < cm) { dr = a; break; } } const has = unlockedAuras.includes(dr.id); if (has) { totalKeys += 3; saveAllData(); const sk2 = document.getElementById('shopKeyCount'); if(sk2) sk2.textContent = totalKeys; showResult('Дубликат ауры: ' + dr.name, 'Возврат: +3 🔑', dr.color); } else { unlockedAuras.push(dr.id); saveAllData(); showResult('Новая аура: ' + dr.name + '!', dr.color); } if (btn) btn.disabled = false; renderAuras(); }, 1200); }
function showResult(t, tx, cl) {
    const rt = document.getElementById('resultTitle'), rx = document.getElementById('resultText'), cr = document.getElementById('chestResult');
    if(rt) rt.textContent = t; if(rx) rx.textContent = tx;
    let box = document.getElementById('resultColorBox');
    if (!box) { box = document.createElement('div'); box.id = 'resultColorBox'; box.style.cssText = 'width:32px;height:32px;border-radius:6px;margin:5px auto 8px;border:2px solid #fff;box-shadow:0 0 8px rgba(0,0,0,0.5);'; if(cr) cr.insertBefore(box, cr.querySelector('h3')); }
    box.style.background = cl || '#555';
    if (cl && cr.querySelector('h3')) cr.querySelector('h3').style.color = cl;
    if(cr) cr.style.display = 'block';
}
function hideResult() { const cr = document.getElementById('chestResult'); if(cr) cr.style.display = 'none'; }

// ==================== ЭФФЕКТ АУРЫ (БЕЗОПАСНЫЙ, С КАРТИНКОЙ) ====================
function showAuraEffectOnPlayer(x, y, aura) {
    if (activeAuraEffect) { activeAuraEffect.remove(); activeAuraEffect = null; }
    const e = document.createElement('div'); e.className = 'aura-effect player-aura';
    ...
    if (aura.id === 'batidao_aura' && aura.image) {
        e.style.background = `radial-gradient(circle, ${aura.effectColor} 0%, transparent 70%)`;
        e.style.backgroundImage = `url('${aura.image}')`;
        e.style.backgroundSize = 'contain'; e.style.backgroundPosition = 'center'; e.style.backgroundRepeat = 'no-repeat';
        e.style.backgroundBlendMode = 'screen'; e.style.boxShadow = `0 0 50px ${aura.color}, 0 0 100px ${aura.color}`;
        e.style.animation = 'batidaoAuraPlayer 0.5s ease-out forwards';
        for (let i = 0; i < 30; i++) { setTimeout(() => { const p = document.createElement('div'); p.style.cssText = `position:fixed;pointer-events:none;z-index:199;left:${x + (Math.random() - 0.5) * 180}px;top:${y + (Math.random() - 0.5) * 180}px;width:${Math.random() * 8 + 4}px;height:${Math.random() * 8 + 4}px;background:#ff0000;border-radius:50%;box-shadow:0 0 15px ${aura.color};animation:particleExplode 0.5s ease-out forwards`; document.body.appendChild(p); setTimeout(() => p.remove(), 500); }, i * 10); }
    } else { e.style.background = `radial-gradient(circle, ${aura.effectColor}, transparent)`; e.style.boxShadow = `0 0 40px ${aura.color}`; e.style.animation = 'auraExpandPlayer 0.4s ease-out forwards'; }
...
class Player {
    ...
    reset() {
        ...
        this.meleeRadius = c.meleeRadius || 95; this.rotation = 0; this.prevY = this.y;
        return this;
    }
    update(k) {
        this.prevY = this.y;
        if (this.invulnerable > 0) this.invulnerable--; if (this.dashCooldown > 0) this.dashCooldown--;
        if (this.meleeCooldown > 0) this.meleeCooldown--; if (this.swingEffect > 0) this.swingEffect--;
        ...
        if ((k[' '] || k['ArrowUp'] || k['w'] || k['ц']) && this.jumpCount < this.maxJumps) {
            if (!this.jumping || this.maxJumps > 1) { this.velY = -this.jumpPower; this.jumping = true; this.jumpCount++; this.createJumpParticles(); AudioSys.jump(); k[' '] = k['ArrowUp'] = k['w'] = k['ц'] = false; }
        }
    ...
    shootArrow() { if (equippedClass !== 'archer') return; AudioSys.arrowShoot(); arrows.push(new Arrow(this.x + (this.dashDirection > 0 ? this.width : -10), this.y + this.height / 2, this.dashDirection, 1)); for (let i = 0; i < 8; i++) particlePool.acquire(this.x + this.width / 2, this.y + this.height / 2, '#27ae60'); }
    startDash() { this.isDashing = true; this.dashTimer = 12; this.dashCharges--; this.dashCooldown = 45; this.invulnerable = 15; this.velY = 0; AudioSys.dash(); for (let i = 0; i < 20; i++) particlePool.acquire(this.x + this.width / 2 + Math.random() * 20 - 10, this.y + this.height / 2 + Math.random() * 20 - 10, '#FFDE7D'); updateDashIndicator(); }
    meleeAttack() {
        ...
        const check = (e) => {
            ...
            if (d < this.meleeRadius) {
                if (e.takeDamage(this.meleeDamage)) {
                    ...
                    else if (e === boss) { boss = null; const bh = document.getElementById('bossHealthBar'); if(bh) bh.style.display = 'none'; }
                }
    ...
    checkPlatformCollisions() {
        let onGround = false;
        for (let p of platforms) {
            if (this.x + this.width <= p.x + 4 || this.x >= p.x + p.width - 4) continue;
            if (this.velY >= 0) {
                const wasAbove = this.prevY + this.height <= p.y + 8;
    ...
    checkLavaDamage() { for (let lp of lavaPlatforms) { if (this.x < lp.x + lp.width && this.x + this.width > lp.x && this.y + this.height > lp.y - 10 && this.y < lp.y + lp.height) { if (Date.now() - (lp.lastDamage || 0) > 900) { this.takeDamage(Math.floor(maxHealth * 0.25), 0, -5, '#ff4500'); lp.lastDamage = Date.now(); AudioSys.lavaBurn(); for (let i = 0; i < 10; i++) particlePool.acquire(this.x + Math.random() * this.width, this.y + this.height, '#ff4500'); } } } }
    checkCollision(o) { return this.x < o.x + o.width && this.x + this.width > o.x && this.y < o.y + o.height && this.y + this.height > o.y; }
    takeDamage(a, kx, ky, cl) { if (this.invulnerable > 0 || this.isDashing) return false; playerHealth = Math.max(0, playerHealth - a); updateHealthBar(); this.knockback(kx, ky); this.invulnerable = 45; AudioSys.hit(); roundDamage += a; for (let i = 0; i < 15; i++) particlePool.acquire(this.x + this.width / 2, this.y + this.height / 2, cl || '#ff2e63'); if (playerHealth <= 0) { gameOver(); return true; } return false; }
    ...
    draw(ctx, cx) {
        ...
        for (let i = 0; i < this.trail.length; i++) { ctx.globalAlpha = i / this.trail.length * 0.25; ctx.fillStyle = this.isDashing ? '#FFDE7D' : skin.color; if (skin.type === 'sphere') { ctx.beginPath(); ctx.arc(this.trail[i].x - cx + 20, this.trail[i].y + 20, 20, 0, Math.PI * 2); ctx.fill(); } else ctx.fillRect(this.trail[i].x - cx, this.trail[i].y, 40, 40); }
        ctx.globalAlpha = 1; const dx = this.x - cx;
        if (skin.type === 'sphere') { ctx.save(); ctx.translate(dx + 20, this.y + 20); ctx.rotate(this.rotation); const g = ctx.createRadialGradient(-8, -8, 2, 0, 0, 20); g.addColorStop(0, skin.color); g.addColorStop(0.6, skin.color + '88'); g.addColorStop(1, skin.color + '44'); ctx.fillStyle = g; ctx.beginPath(); ctx.arc(0, 0, 20, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = 'rgba(255,255,255,0.4)'; ctx.beginPath(); ctx.arc(-6, -6, 5, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#222'; ctx.beginPath(); ctx.arc(-7, -3, 4, 0, Math.PI * 2); ctx.arc(7, -3, 4, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(-6, -4, 2, 0, Math.PI * 2); ctx.arc(8, -4, 2, 0, Math.PI * 2); ctx.fill(); ctx.restore(); }
        else if (skin.type === 'decorated') { ctx.fillStyle = skin.color; ctx.fillRect(dx, this.y, 40, 40); ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; if (skin.pattern === 'stripes') { for (let i = 0; i < 40; i += 8) ctx.fillRect(dx + i, this.y, 4, 40); } else if (skin.pattern === 'marble') { ctx.fillStyle = '#ecf0f1'; ctx.fillRect(dx, this.y, 40, 40); ctx.strokeStyle = '#95a5a6'; ctx.lineWidth = 1; for (let i = 0; i < 5; i++) { ctx.beginPath(); ctx.moveTo(dx, this.y + i*10); ctx.quadraticCurveTo(dx+20, this.y+i*10+5, dx+40, this.y+i*10); ctx.stroke(); } } else if (skin.pattern === 'neon') { ctx.shadowColor = skin.color; ctx.shadowBlur = 15; ctx.strokeRect(dx, this.y, 40, 40); ctx.shadowBlur = 0; } ctx.fillStyle = '#222'; ctx.fillRect(dx + 10, this.y + 10, 8, 8); ctx.fillRect(dx + 22, this.y + 10, 8, 8); }
        else if (skin.type === 'pyramid') { ctx.fillStyle = skin.color; ctx.beginPath(); ctx.moveTo(dx + 20, this.y); ctx.lineTo(dx + 40, this.y + 40); ctx.lineTo(dx, this.y + 40); ctx.fill(); ctx.fillStyle = '#222'; ctx.beginPath(); ctx.arc(dx + 14, this.y + 22, 4, 0, Math.PI * 2); ctx.arc(dx + 26, this.y + 22, 4, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(dx + 15, this.y + 21, 2, 0, Math.PI * 2); ctx.arc(dx + 27, this.y + 21, 2, 0, Math.PI * 2); ctx.fill(); }
        else if (skin.type === 'diamond') { ctx.save(); ctx.translate(dx + 20, this.y + 20); ctx.rotate(Math.PI / 4); ctx.fillStyle = skin.color; ctx.fillRect(-20, -20, 40, 40); ctx.restore(); ctx.fillStyle = '#222'; ctx.save(); ctx.translate(dx + 20, this.y + 20); ctx.rotate(Math.PI / 4); ctx.fillRect(-12, -8, 6, 6); ctx.fillRect(6, -8, 6, 6); ctx.restore(); }
        else { ctx.fillStyle = skin.color; if (this.isDashing) { ctx.shadowColor = skin.color; ctx.shadowBlur = 20; } ctx.fillRect(dx, this.y, 40, 40); ctx.shadowBlur = 0; ctx.fillStyle = '#222'; ctx.fillRect(dx + 10, this.y + 10, 8, 8); ctx.fillRect(dx + 22, this.y + 10, 8, 8); ctx.fillRect(dx + 10, this.y + 25, 20, 4); }
        ctx.strokeStyle = this.invulnerable > 0 && Math.floor(this.invulnerable / 4) % 2 === 0 ? '#fff' : skin.color; ctx.lineWidth = 2; if (skin.type === 'cube') ctx.strokeRect(dx, this.y, 40, 40);
...
// ==================== БОСС ====================
class Boss { constructor(x,y){this.x=x;this.y=y;this.width=40;this.height=40;this.maxHealth=CONFIG.boss.health+Math.floor(currentLevel/5)*5;this.health=this.maxHealth;this.speed=CONFIG.boss.moveSpeed;this.dir=1;this.cd=0;this.active=true;this.tex=bossTextures.getRandom(enCol);this.phase=0;this.mp=0;this.sp=0;this.color=this.tex.type==='color'?this.tex.color:'#ff0000';}update(){if(!this.active)return;this.mp+=0.02;this.cd--;if(Math.abs(this.x-player.x)>200){this.x+=this.speed*this.dir;if(this.x<cameraX+100||this.x>cameraX+canvas.width-140)this.dir*=-1;}this.y+=Math.sin(this.mp)*2;if(this.cd<=0){this.attack();this.cd=120-(currentLevel*2);}if(player.checkCollision(this)){if(player.takeDamage(30,this.x<player.x?15:-15,-10,'#ff0000'))return;}}attack(){this.sp=(this.sp+1)%3;if(this.sp===0)for(let i=0;i<15;i++)particlePool.acquire(this.x+20,this.y+20,'#ff0000');else if(this.sp===1)for(let i=-1;i<=1;i++)particlePool.acquire(this.x+20,this.y+20,0,4,10,'#ff6600',false);else for(let i=0;i<8;i++){const a=(i/8)*Math.PI*2;particlePool.acquire(this.x+20,this.y+20,Math.cos(a)*6,Math.sin(a)*6,8,'#ff0000');}for(let i=0;i<15;i++)particlePool.acquire(this.x+20,this.y+20,'#ff0000');}takeDamage(d=1){this.health-=d;AudioSys.bossHit();for(let i=0;i<10;i++)particlePool.acquire(this.x+Math.random()*40,this.y+Math.random()*40,'#ff0000');if(this.health<=0){this.defeat();return true;}updateBossHealth();return false;}defeat(){this.active=false;AudioSys.bossDefeat();bossesDefeated++;for(let i=0;i<100;i++)particlePool.acquire(this.x+20,this.y+20,this.color);addScore(5000*comboMultiplier);updateCombo();const bh=document.getElementById('bossHealthBar');if(bh)bh.style.display='none';if(Math.random()<0.8)powerUps.push({x:this.x+20,y:this.y+20,size:20,color:'#FF2E63',type:'health'});if(Math.random()<0.5)powerUps.push({x:this.x+60,y:this.y+20,size:20,color:'#FFDE7D',type:'dash'});shakeScreen(10);}draw(ctx){if(!this.active)return;const dx=this.x-cameraX;ctx.shadowColor='#ff0000';ctx.shadowBlur=30;if(this.tex.type==='image'&&this.tex.image){try{ctx.drawImage(this.tex.image,dx,this.y,40,40);}catch(e){ctx.fillStyle=this.color;ctx.fillRect(dx,this.y,40,40);}}else{ctx.fillStyle=this.color;ctx.fillRect(dx,this.y,40,40);ctx.fillStyle='rgba(0,0,0,0.3)';for(let i=0;i<40;i+=20)for(let j=0;j<40;j+=20)if((i/20+j/20)%2===0)ctx.fillRect(dx+i,this.y+j,20,20);}ctx.shadowBlur=0;ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(dx+10,this.y+15,5,0,Math.PI*2);ctx.arc(dx+30,this.y+15,5,0,Math.PI*2);ctx.fill();ctx.fillStyle='#000';ctx.beginPath();ctx.arc(dx+10+(this.dir*2),this.y+15,2,0,Math.PI*2);ctx.arc(dx+30+(this.dir*2),this.y+15,2,0,Math.PI*2);ctx.fill();if(this.health<this.maxHealth*0.3&&Math.floor(Date.now()/200)%2===0){ctx.strokeStyle='#ff0000';ctx.lineWidth=4;ctx.strokeRect(dx-5,this.y-5,50,50);}}}
...
// ==================== ВИХРЬ ====================
class VortexEnemy{constructor(x,y){this.x=x;this.y=y;this.oy=y;this.width=40;this.height=40;this.color='#00ffff';this.health=4;this.maxHealth=4;this.active=true;this.rot=0;this.fp=Math.random()*Math.PI*2;this.shootCooldown=0;this.shootInterval=45;}update(){if(!this.active)return;this.rot+=0.1;this.fp+=0.03;this.y=this.oy+Math.sin(this.fp)*15;this.shootCooldown--;if(this.shootCooldown<=0&&Math.abs(this.x-player.x)<400&&Math.abs(this.y-player.y)<300){windProjectiles.push(new WindProjectile(this.x+20,this.y+20,player.x+20,player.y+20));AudioSys.windShoot();this.shootCooldown=this.shootInterval;for(let i=0;i<5;i++)particlePool.acquire(this.x+20,this.y+20,'#00ffff');}const dx=player.x-this.x,dy=player.y-this.y,d=Math.sqrt(dx*dx+dy*dy);if(d<250&&d>30){player.velX+=(dx/d)*0.8;player.velY+=(dy/d)*0.4;}if(Math.abs(this.x-player.x)>150)this.x+=(player.x>this.x?0.3:-0.3);}takeDamage(d=1){this.health-=d;if(this.health<=0){this.destroy();return true;}return false;}destroy(){this.active=false;addScore(250*comboMultiplier);updateCombo();AudioSys.collect();for(let i=0;i<40;i++)particlePool.acquire(this.x+20,this.y+20,'#00ffff');}draw(ctx){if(!this.active)return;const dx=this.x-cameraX;ctx.save();ctx.translate(dx+20,this.y+20);ctx.rotate(this.rot);for(let i=0;i<4;i++){ctx.fillStyle=`rgba(0,255,255,${0.3+i*0.15})`;ctx.beginPath();ctx.arc(0,0,20-i*5,i*0.5,i*0.5+Math.PI);ctx.fill();}ctx.restore();ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(dx+13,this.y+15,5,0,Math.PI*2);ctx.arc(dx+27,this.y+15,5,0,Math.PI*2);ctx.fill();ctx.fillStyle='#000';ctx.beginPath();ctx.arc(dx+14,this.y+16,2,0,Math.PI*2);ctx.arc(dx+28,this.y+16,2,0,Math.PI*2);ctx.fill();ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(dx+20,this.y+20,6,0,Math.PI*2);ctx.fill();if(this.health<this.maxHealth){ctx.fillStyle='#4af626';ctx.fillRect(dx,this.y-12,(40*this.health)/this.maxHealth,4);}ctx.fillStyle='#fff';ctx.font='bold 11px Unbounded';ctx.textAlign='center';ctx.fillText('ВИХРЬ',dx+20,this.y-20);}}

// ==================== БОМЖ С ЛАВОВОЙ АТАКОЙ ====================
class HomelessEnemy{constructor(x,y,plat){this.x=x;this.y=y;this.width=40;this.height=40;this.color='#8B7355';this.health=5;this.maxHealth=5;this.active=true;this.plat=plat;this.bw=0;this.lavaCooldown=0;this.lavaInterval=600;}update(){if(!this.active)return;this.bw+=0.15;this.y=this.plat.y-this.height;this.x=this.plat.x+this.plat.width/2-20;this.lavaCooldown--;if(this.lavaCooldown<=0&&Math.abs(this.x-player.x)<400){AudioSys.homelessShoot();for(let i=0;i<8;i++){const angle=(i/8)*Math.PI*2-Math.PI/2;lavaProjectiles.push(new LavaProjectile(this.x+20,this.y+10,angle));}this.lavaCooldown=this.lavaInterval;for(let i=0;i<15;i++)particlePool.acquire(this.x+20,this.y+10,'#ff4500');}}takeDamage(d=1,s){this.health-=(s==='arrow'?d*1.67:d);if(this.health<=0){this.destroy();return true;}return false;}destroy(){this.active=false;addScore(180*comboMultiplier);updateCombo();AudioSys.collect();for(let i=0;i<25;i++)particlePool.acquire(this.x+20,this.y+20,'#8B7355');if(Math.random()<0.15)coins.push({x:this.x+20,y:this.y-20,size:12,color:'#FFDE7D',bounce:0});}draw(ctx){if(!this.active)return;const dx=this.x-cameraX;ctx.fillStyle=this.color;ctx.fillRect(dx,this.y+5,40,35);ctx.fillStyle='#D2B48C';ctx.beginPath();ctx.arc(dx+20,this.y+10,12,0,Math.PI*2);ctx.fill();ctx.fillStyle='#6B5443';ctx.beginPath();ctx.moveTo(dx+8,this.y+12);for(let i=0;i<6;i++)ctx.quadraticCurveTo(dx+12+i*4,this.y+20+Math.sin(this.bw+i)*3,dx+14+i*4,this.y+12);ctx.lineTo(dx+32,this.y+12);ctx.fill();ctx.fillStyle='#000';ctx.beginPath();ctx.arc(dx+15,this.y+8,3,0,Math.PI*2);ctx.arc(dx+25,this.y+8,3,0,Math.PI*2);ctx.fill();ctx.fillStyle='#4a3728';ctx.fillRect(dx+5,this.y-2,30,8);ctx.fillRect(dx+2,this.y-6,36,4);if(this.health<this.maxHealth){ctx.fillStyle='#4af626';ctx.fillRect(dx,this.y-15,(40*this.health)/this.maxHealth,4);}ctx.fillStyle='#fff';ctx.font='bold 11px Unbounded';ctx.textAlign='center';ctx.fillText('БОМЖ',dx+20,this.y-22);if(this.lavaCooldown>0){ctx.fillStyle=`rgba(255,69,0,${0.3+0.4*Math.sin(Date.now()/100)})`;ctx.beginPath();ctx.arc(dx+20,this.y-25,8+Math.sin(Date.now()/150)*2,0,Math.PI*2);ctx.fill();}}}

// ==================== ПЛАТФОРМА ====================
class Platform{constructor(x,y,w,ti,lv=false){this.x=x;this.y=y;this.width=w;this.height=25;this.tex=platTex[ti];this.glow=Math.random()>0.7;this.gp=Math.random()*Math.PI*2;this.type=Math.random()>0.8?(Math.random()>0.5?'moving':'breaking'):'normal';this.md=1;this.ms=1;this.ox=x;this.bt=0;this.broken=false;this.isLava=lv;}update(){if(this.glow)this.gp+=0.05;if(this.type==='moving'&&!this.broken){this.x+=this.ms*this.md;if(Math.abs(this.x-this.ox)>100)this.md*=-1;}if(this.type==='breaking'&&this.bt>0){this.bt--;if(this.bt===0){this.broken=true;setTimeout(()=>this.broken=false,3000);}}}draw(ctx){if(this.broken)return;ctx.fillStyle='rgba(0,0,0,0.25)';ctx.fillRect(this.x-cameraX+3,this.y+3,this.width,this.height);if(this.isLava){ctx.fillStyle='#ff4500';ctx.fillRect(this.x-cameraX,this.y+5,this.width,this.height-5);ctx.fillStyle='#ff6600';for(let i=0;i<this.width;i+=15){ctx.beginPath();ctx.arc(this.x-cameraX+i+7,this.y+10,4,0,Math.PI);ctx.fill();}}else{ctx.fillStyle=this.tex.c;if(this.type==='breaking'&&this.bt>0)ctx.globalAlpha=Math.sin(Date.now()/100)*0.3+0.7;ctx.fillRect(this.x-cameraX,this.y,this.width,this.height);ctx.globalAlpha=1;ctx.fillStyle='rgba(255,255,255,0.12)';const p=this.tex.p;if(p==='stripes'){for(let i=0;i<this.width;i+=30)ctx.fillRect(this.x-cameraX+i,this.y,15,this.height);}else if(p==='dots'){for(let i=6;i<this.width;i+=12)for(let j=6;j<this.height;j+=12){ctx.beginPath();ctx.arc(this.x-cameraX+i,this.y+j,2,0,Math.PI*2);ctx.fill();}}else if(p==='checker'){for(let i=0;i<this.width;i+=8)for(let j=0;j<this.height;j+=8)if((Math.floor(i/8)+Math.floor(j/8))%2===0)ctx.fillRect(this.x-cameraX+i,this.y+j,8,8);}else if(p==='zigzag'){for(let i=0;i<this.width;i+=20){ctx.beginPath();ctx.moveTo(this.x-cameraX+i,this.y+this.height);ctx.lineTo(this.x-cameraX+i+10,this.y);ctx.lineTo(this.x-cameraX+i+20,this.y+this.height);ctx.fill();}}else if(p==='bricks'){for(let i=0;i<this.width;i+=25)for(let j=0;j<this.height;j+=12)if((i/25+j/12)%2===0)ctx.fillRect(this.x-cameraX+i,this.y+j,25,12);}else if(p==='waves'){for(let i=0;i<this.width;i+=30){ctx.beginPath();ctx.moveTo(this.x-cameraX+i,this.y+this.height/2);for(let j=0;j<30;j+=5){const x=this.x-cameraX+i+j,y=this.y+this.height/2+Math.sin(j/30*Math.PI*2)*5;ctx.lineTo(x,y);}ctx.lineTo(this.x-cameraX+i+30,this.y+this.height);ctx.lineTo(this.x-cameraX+i,this.y+this.height);ctx.fill();}}}if(this.glow&&!this.isLava){const gi=Math.sin(this.gp)*0.3+0.7;ctx.shadowColor=this.tex.c;ctx.shadowBlur=15*gi;ctx.strokeStyle=this.tex.c;ctx.lineWidth=2;ctx.strokeRect(this.x-cameraX,this.y,this.width,this.height);ctx.shadowBlur=0;}ctx.strokeStyle='rgba(0,0,0,0.25)';ctx.lineWidth=2;ctx.strokeRect(this.x-cameraX,this.y,this.width,this.height);if(this.type==='breaking'){ctx.fillStyle='#ff0000';for(let i=0;i<3;i++){ctx.beginPath();ctx.arc(this.x-cameraX+this.width/4+i*(this.width/4),this.y+5,3,0,Math.PI*2);ctx.fill();}}if(this.type==='moving'){ctx.fillStyle='#0088ff';const ac=Math.floor(this.width/30);for(let i=0;i<ac;i++){const ax=this.x-cameraX+15+i*30;ctx.beginPath();ctx.moveTo(ax,this.y+this.height/2);ctx.lineTo(ax+10*this.md,this.y+10);ctx.lineTo(ax+10*this.md,this.y+this.height-10);ctx.closePath();ctx.fill();}}}}

// ==================== ГЕНЕРАЦИЯ УРОВНЯ ====================
function generateLevel(level){
    platforms=[];enemies=[];flyingEnemies=[];homelessEnemies=[];necromancers=[];vortexes=[];coins=[];powerUps=[];levelKeys=[];deadEnemiesForNecro=[];windProjectiles=[];lavaProjectiles=[];
    ...
    if(level>=3){const safePlat=platforms[Math.floor(Math.random()*(platforms.length-5))+3];const lp=new Platform(safePlat.x-30,safePlat.y,110,Math.floor(Math.random()*platTex.length),true);platforms.push(lp);lavaPlatforms.push(lp);homelessEnemies.push(new HomelessEnemy(safePlat.x+15,safePlat.y-40,lp));}
    const newTypes=['necromancer','vortex'];
    for(let i=0;i<ec;i++){
        const pi=Math.floor(Math.random()*(platforms.length-5))+3;const p=platforms[pi];
        if(level>=3&&i>0&&i%4===0){const t=newTypes[Math.floor(Math.random()*newTypes.length)];if(t==='necromancer')necromancers.push(new NecromancerEnemy(p.x+p.width/2-20,p.y-40));else vortexes.push(new VortexEnemy(p.x+p.width/2-20,150+Math.random()*200));}
...
function updateLavaProjectiles(){for(let i=lavaProjectiles.length-1;i>=0;i--){lavaProjectiles[i].update();if(!lavaProjectiles[i].active)lavaProjectiles.splice(i,1);}}
...
function drawLavaProjectiles(){lavaProjectiles.forEach(l=>l.draw(ctx));}
...
function gameLoop(){
    ...
    try {
        ...
        updateArrows(); updateWindProjectiles(); updateLavaProjectiles(); updateNewEnemies(); decayCombo();
        ...
        drawKeys();drawCoins();drawPowerUps();drawArrows();drawWindProjectiles();drawLavaProjectiles();drawParticles();
        player.draw(ctx,cameraX);drawGoal();
        ctx.setTransform(1,0,0,1,0,0);updateDashIndicator();
    } catch(e) { console.warn('Ошибка в цикле:', e); }
    requestAnimationFrame(gameLoop);
}

function completeLevel(){
    gameRunning=false;AudioSys.levelComplete();addScore(1000*currentLevel);const er=calculateEloChange();
    const ls=document.getElementById('levelScore');if(ls)ls.textContent=Math.floor(1000*currentLevel*comboMultiplier);
    const mc=document.getElementById('maxCombo');if(mc)mc.textContent=`x${maxCombo}`;
    const cc=document.getElementById('coinsCollected');if(cc)cc.textContent=roundCoins;
    const dt=document.getElementById('damageTaken');if(dt)dt.textContent=roundDamage;
    const el=document.getElementById('eloChangeDisplay');if(el){el.textContent=(er.change>=0?'+':'')+er.change;el.style.color=er.change>=0?'#4af626':'#ff2e63';}
    const lc=document.getElementById('levelComplete');if(lc)lc.style.display='flex';showEloChange(er.change);
    for(let i=0;i<80;i++)particlePool.acquire(player.x+20,player.y+20,platTex[Math.floor(Math.random()*platTex.length)].c);
    setTimeout(()=>{if(lc)lc.style.display='none';currentLevel++;generateLevel(currentLevel);player.reset();playerHealth=maxHealth;updateHealthBar();cameraX=0;comboCount=1;comboMultiplier=1;maxCombo=1;lastCheckpointX=0;updateUI();updateDashIndicator();updateEloDisplay();gameRunning=true;gameLoop();},2500);
}

function gameOver(){
    gameRunning=false;AudioSys.gameOver();
    const fs=document.getElementById('finalScore');if(fs)fs.textContent=score;
    const fl=document.getElementById('finalLevel');if(fl)fl.textContent=currentLevel-1;
    const fc=document.getElementById('finalCombo');if(fc)fc.textContent=`x${maxCombo}`;
    const bd=document.getElementById('bossesDefeated');if(bd)bd.textContent=bossesDefeated;
    const fe=document.getElementById('finalElo');if(fe)fe.textContent=Math.floor(playerELO);
...
function restartGame(){
    ...
    const cs=document.getElementById('caseShopScreen');if(cs)cs.style.display='none';
    currentLevel=1;score=0;playerHealth=getCurrentClass().health||100;maxHealth=playerHealth;
    comboCount=1;maxCombo=1;comboMultiplier=1;lastCheckpointX=0;bossesDefeated=0;roundCoins=0;roundDamage=0;
    arrows=[];windProjectiles=[];lavaProjectiles=[];deadEnemiesForNecro=[];
    generateLevel(currentLevel);player=new Player();cameraX=0;updateUI();updateHealthBar();updateDashIndicator();updateEloDisplay();
    const bh=document.getElementById('bossHealthBar');if(bh)bh.style.display='none';gameRunning=true;gameLoop();
}
...
function showPauseMenu(){gameRunning=false;const p=document.getElementById('pauseMenu');if(p)p.style.display='flex';}
...
if(canvas)canvas.addEventListener('mousedown',e=>{if(e.button===0&&gameRunning)performMelee();});

function setupMobileControls(){
    const isT='ontouchstart'in window||navigator.maxTouchPoints>0;
    if(isT||window.innerWidth<=768){
        const mc=document.getElementById('mobileControls');if(mc)mc.style.display='flex';
        const bind=(id,key)=>{const b=document.getElementById(id);if(!b)return;b.addEventListener('touchstart',e=>{e.preventDefault();keys[key]=true;});b.addEventListener('touchend',e=>{e.preventDefault();keys[key]=false;});b.addEventListener('mousedown',()=>keys[key]=true);b.addEventListener('mouseup',()=>keys[key]=false);b.addEventListener('mouseleave',()=>keys[key]=false);};
        bind('btnLeft','ArrowLeft');bind('btnRight','ArrowRight');bind('btnJump',' ');bind('btnDash','Shift');
        const ba=document.getElementById('btnAttack');if(ba){ba.addEventListener('touchstart',e=>{e.preventDefault();performMelee();});ba.addEventListener('mousedown',performMelee);}
        const bs=document.getElementById('btnShoot');if(bs){bs.addEventListener('touchstart',e=>{e.preventDefault();if(equippedClass==='archer'&&arrowCooldown<=0){player.shootArrow();arrowCooldown=25;}});bs.addEventListener('mousedown',()=>{if(equippedClass==='archer'&&arrowCooldown<=0){player.shootArrow();arrowCooldown=25;}});}
    }
}

const st=document.getElementById('soundToggle');if(st)st.addEventListener('change',e=>{CONFIG.audio.enabled=e.target.checked;AudioSys.enabled=e.target.checked;});
const pt=document.getElementById('particlesToggle');if(pt)pt.addEventListener('change',e=>{CONFIG.particles.enabled=e.target.checked;if(!e.target.checked)particlePool?.releaseAll();});

async function initGame(){
    resizeCanvas();
    if(!ctx){console.error('Canvas не найден');return;}
    await bossTextures.load();
    particlePool=new ObjectPool((x,y,c)=>new Particle(x,y,c),CONFIG.particles.maxCount);
    let pr=0;const pi=setInterval(()=>{pr+=Math.random()*15;const pf=document.getElementById('progressFill');if(pf)pf.style.width=`${Math.min(pr,100)}%`;if(pr>=100){clearInterval(pi);setTimeout(()=>{const ld=document.getElementById('loading');if(ld){ld.style.opacity='0';setTimeout(()=>ld.style.display='none',500);}},300);}},100);
    generateLevel(currentLevel);player=new Player();applyClassStats();updateUI();updateHealthBar();updateDashIndicator();updateEloDisplay();setupMobileControls();
...
window.onerror = function(msg, url, line) { console.warn('Global JS Error:', msg, line); return true; };
