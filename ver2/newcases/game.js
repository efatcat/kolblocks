...
class WindProjectile {
    constructor(x, y, targetX, targetY) {
        ...
        this.life = 100; this.color = '#00ffff';
...
function openChestType(id, btn) {
    ...
    if (ch.isAura) {
        ...
        setTimeout(() => {
            ...
            if (has) { totalKeys += 3; saveAllData(); document.getElementById('shopKeyCount').textContent = totalKeys; showResult('Дубликат ауры: ' + dr.name, 'Возврат: +3 🔑', dr.color); }
    ...
    setTimeout(() => {
        ...
        if (has) { totalKeys += Math.floor(ch.price * 0.2); saveAllData(); document.getElementById('shopKeyCount').textContent = totalKeys; showResult('Дубликат: ' + dr.name, 'Возврат: +' + Math.floor(ch.price * 0.2) + ' 🔑', dr.color); }
...
function generateLevel(level){
    ...
    // Убрана вспомогательная платформа, оставлена только лавовая
