...
const PLAYER_CLASSES = [
    ...
    },
    {
        id: 'archer',
        name: 'Лучник',
        description: '🏹 Дальняя атака стрелами',
        color: '#27ae60',
        price: 35
    },
    {
        id: 'mage',
        name: 'Маг',
        description: '🔮 Падает медленнее, +35% радиус',
        color: '#8e44ad',
        price: 85
    },
    {
        id: 'rogue',
        name: 'Разбойник',
        description: '⚡ +40% скорость, тройной прыжок',
        color: '#f39c12',
        price: 90
    }
];
...
const CHEST_SKINS = [
    ...
    },
    {
        ...
        type: 'decorated',
        pattern: 'stripes',
        texture: 'https://kovka-deshevo.ru/wp-content/uploads/2022/03/uzor-N29-kupit-v-tule-moskve-cena.jpg'
    },
    {
        id: 'cube_marble',
        name: 'Мраморный куб',
...
const AURA_SKINS = [
    ...
    },
    {
        ...
        name: 'Ледяная аура',
        color: '#00ccff',
        effectColor: 'rgba(0, 204, 255, 0.6)',
        chance: 30
    },
    {
        id: 'lightning_aura',
        name: 'Электрическая аура',
        color: '#ffff00',
        effectColor: 'rgba(255, 255, 0, 0.6)',
        chance: 20
    },
    {
        id: 'cosmic_aura',
    ...
    },
    {
        ...
        name: 'Но батидао',
        color: '#ff0000',
        effectColor: 'rgba(255, 0, 0, 0.8)',
        chance: 1,
        image: 'https://images.genius.com/3849b06fe11fa1c89ba96465b298457c.1000x1000x1.png'
    }
];
...
const CHEST_TYPES = [
    {
        ...
        price: 10,
        icon: '📦',
        skins: CHEST_SKINS.filter(s => ['copper', 'sapphire', 'magma', 'royal'].includes(s.id))
    },
    {
        id: 'shapes',
        name: 'Кейс форм',
        price: 25,
        icon: '🔷',
        skins: CHEST_SKINS.filter(s => ['sphere_fire', 'sphere_ice', 'sphere_cosmic', 'pyramid_gold', 'diamond'].includes(s.id))
    },
    {
        id: 'decorated',
        name: 'Узорчатый кейс',
        price: 30,
        icon: '✨',
        skins: CHEST_SKINS.filter(s => ['cube_decorated', 'cube_marble', 'cube_neon'].includes(s.id))
    },
    {
        id: 'aura',
        name: 'Кейс аур',
...
const AudioSys = {
    ...
    levelComplete() {
        [523, 659, 784, 1047].forEach((f, i) => {
            setTimeout(() => this.play(f, 0.2, 'sine', 0.15), i * 100);
        });
    },
    
    bossSpawn() {
        [200, 250, 300, 250, 200].forEach((f, i) => {
            setTimeout(() => this.play(f, 0.3, 'sawtooth', 0.2, 0.5), i * 100);
        });
    },
    ...
    bossDefeat() {
        [400, 500, 600, 800, 1000].forEach((f, i) => {
            setTimeout(() => this.play(f, 0.4, 'square', 0.2), i * 150);
        });
    },
    
    gameOver() {
        [392, 349, 294, 262].forEach((f, i) => {
            setTimeout(() => this.play(f, 0.3, 'sawtooth', 0.15, 0.5), i * 150);
        });
    },
    ...
    chestOpen() {
        [300, 450, 600, 900].forEach((f, i) => {
            setTimeout(() => this.play(f, 0.15, 'square', 0.15), i * 80);
        });
    },
...
class Arrow {
    ...
    update() {
        ...
        for (let e of targets) {
            ...
            if (this.x < e.x + e.width && this.x + this.w > e.x && 
                this.y < e.y + e.height && this.y + this.h > e.y) {
                
                if (e.takeDamage(this.dmg, 'arrow')) {
                    if (e instanceof Enemy) {
                        enemies = enemies.filter(x => x !== e);
                    } else if (e instanceof FlyingEnemy) {
                        flyingEnemies = flyingEnemies.filter(x => x !== e);
                    } else if (e instanceof HomelessEnemy) {
                        homelessEnemies = homelessEnemies.filter(x => x !== e);
                    } else if (e instanceof NecromancerEnemy) {
                        necromancers = necromancers.filter(x => x !== e);
                    } else if (e instanceof VortexEnemy) {
                        vortexes = vortexes.filter(x => x !== e);
                    }
                }
                ...
                for (let i = 0; i < 8; i++) {
                    particlePool.acquire(this.x, this.y, '#27ae60');
                }
                return;
            }
        }
        ...
        if (boss && boss.active && 
            this.x < boss.x + boss.width && this.x + this.w > boss.x && 
            this.y < boss.y + boss.height && this.y + this.h > boss.y) {
            
            if (boss.takeDamage(this.dmg * 0.5)) {
                boss = null;
...
class WindProjectile {
    ...
    update() {
        ...
        if (this.x < player.x + player.width && this.x + this.w > player.x && 
            this.y < player.y + player.height && this.y + this.h > player.y) {
            ...
            for (let i = 0; i < 10; i++) {
                particlePool.acquire(this.x, this.y, '#00ffff');
            }
            return;
        }
        
        for (let p of platforms) {
            if (this.x < p.x + p.width && this.x + this.w > p.x && 
                this.y < p.y + p.height && this.y + this.h > p.y) {
...
class LavaProjectile {
    ...
    update() {
        ...
        if (this.x < player.x + player.width && this.x + this.w > player.x && 
            ...
            player.takeDamage(15, this.vx * 0.3, -8, '#ff4500');
            this.active = false;
            AudioSys.lavaBurn();
            
            for (let i = 0; i < 12; i++) {
                particlePool.acquire(this.x, this.y, '#ff4500');
            }
            return;
        }
...
function addScore(p) {
    score += Math.floor(p * comboMultiplier);
    updateUI();
}
...
function updateDashIndicator() {
    ...
    dots.forEach((d, i) => d.classList.toggle('active', i < player.dashCharges));
}

function showCheckpointIndicator() {
    const ci = document.getElementById('checkpointIndicator');
    ci.classList.add('active');
    setTimeout(() => ci.classList.remove('active'), 2000);
}

function getCurrentClass() {
    return CONFIG.classes[equippedClass] || CONFIG.classes.default;
}
...
function applyClassStats() {
    ...
    maxHealth = c.health || 100;
    playerHealth = Math.min(playerHealth, maxHealth);
    player.speed = c.speed || 6;
    player.jumpPower = c.jumpPower || 16;
    player.gravity = c.gravity || 0.8;
    player.meleeDamage = c.meleeDamage || 1;
    player.maxJumps = c.maxJumps || 2;
    player.maxDashes = c.maxDashes || 2;
...
function calculateEloChange() {
    ...
    playerELO = Math.max(0, playerELO + ch);
    saveAllData();
...
function getSkinData(id) {
    return CHEST_SKINS.find(s => s.id === id) || { id: 'default', name: 'Стандарт', color: '#4af626', type: 'cube' };
}
...
function renderChestTypes() {
    const c = document.getElementById('chestsContainer');
    c.innerHTML = '';
...
function openChestType(id, btn) {
    ...
    if (ch.isAura) {
        ...
        setTimeout(() => {
            ...
            if (has) {
                ...
                saveAllData();
                document.getElementById('shopKeyCount').textContent = totalKeys;
                showResult('Дубликат ауры: ' + dr.name, 'Возврат: +3 🔑', dr.color);
            } else {
                unlockedAuras.push(dr.id);
                saveAllData();
                showResult('Новая аура: ' + dr.name + '!', dr.color);
            }
    ...
    setTimeout(() => {
        ...
        if (has) {
            totalKeys += Math.floor(ch.price * 0.2);
            saveAllData();
            document.getElementById('shopKeyCount').textContent = totalKeys;
            showResult('Дубликат: ' + dr.name, 'Возврат: +' + Math.floor(ch.price * 0.2) + ' 🔑', dr.color);
        } else {
            unlockedSkins.push(dr.id);
            saveAllData();
            showResult('Новый скин: ' + dr.name + '!', dr.color);
        }
...
function renderSkins() {
    ...
    [{ id: 'default', name: 'Стандарт', color: '#4af626', type: 'cube' }, ...CHEST_SKINS].forEach(s => {
        ...
        let p = '';
        if (s.type === 'sphere') {
            p = `<div class="skin-preview sphere" style="background:radial-gradient(circle at 30% 30%, ${s.color}, ${s.color}88);"></div>`;
        } else if (s.type === 'decorated') {
            if (s.texture) {
                p = `<div class="skin-preview decorated" style="background-image:url('${s.texture}');background-size:cover;background-position:center;background-color:${s.color};"></div>`;
            } else {
                p = `<div class="skin-preview decorated" style="background:linear-gradient(135deg, ${s.color}, ${s.color}88);"></div>`;
            }
        } else if (s.type === 'pyramid') {
            p = `<div class="skin-preview pyramid" style="border-bottom:35px solid ${s.color};border-left:20px solid transparent;border-right:20px solid transparent;"></div>`;
        } else if (s.type === 'diamond') {
            p = `<div class="skin-preview diamond" style="background:${s.color};transform:rotate(45deg);"></div>`;
        } else {
            p = `<div class="skin-preview" style="background:${u ? s.color : '#333'}"></div>`;
        }
...
function renderAuras() {
    ...
    if (unlockedAuras.length === 0) {
        g.innerHTML = '<div style="color:#aaa;text-align:center;grid-column:1/-1;">Нет аур. Открывайте кейс аур!</div>';
        return;
    }
...
function renderClasses() {
    ...
    PLAYER_CLASSES.forEach(c => {
        const u = unlockedClasses.includes(c.id);
        const e = equippedClass === c.id;
...
function openAuraChest() {
    ...
    const btn = document.getElementById('openAuraChestBtn');
    const chest = document.getElementById('auraChestVisual');
...
function showResult(t, tx, cl) {
    ...
    if (!box) {
        ...
        box.id = 'resultColorBox';
        box.style.cssText = 'width:32px;height:32px;border-radius:6px;margin:5px auto 8px;border:2px solid #fff;box-shadow:0 0 8px rgba(0,0,0,0.5);';
        document.getElementById('chestResult').insertBefore(box, document.getElementById('resultTitle'));
    }
    
    box.style.background = cl || '#555';
    
    if (cl) {
        document.getElementById('chestResult').querySelector('h3').style.color = cl;
    }
...
class Player {
    ...
    reset() {
        ...
        this.color = c.color;
        this.speed = c.speed || 6;
        this.jumpPower = c.jumpPower || 16;
        this.gravity = c.gravity || 0.8;
        this.friction = 0.85;
        ...
        this.invulnerable = 0;
        this.dashCharges = c.maxDashes || 2;
        this.dashCooldown = 0;
        ...
        this.meleeDamage = c.meleeDamage || 1;
        this.maxJumps = c.maxJumps || 2;
        this.meleeRadius = c.meleeRadius || 95;
        this.rotation = 0;
        this.prevY = this.y;
    ...
    update(k) {
        ...
        if ((k['Shift'] || k['shift'] || k['q'] || k['й']) && this.dashCharges > 0 && this.dashCooldown <= 0) {
            this.startDash();
            k['Shift'] = k['shift'] = k['q'] = k['й'] = false;
        }
        
        if (equippedClass === 'archer' && (k['v'] || k['V'] || k['м'] || k['М']) && arrowCooldown <= 0) {
            this.shootArrow();
            arrowCooldown = 25;
            k['v'] = k['V'] = k['м'] = k['М'] = false;
        }
        ...
        this.rotation += Math.abs(this.velX) * 0.1;
    }
    ...
    shootArrow() {
        ...
        for (let i = 0; i < 8; i++) {
            particlePool.acquire(this.x + this.width / 2, this.y + this.height / 2, '#27ae60');
        }
    }
    ...
    meleeAttack() {
        ...
        if (equippedAura) {
            const a = getAuraData(equippedAura);
            if (a) showAuraEffectOnPlayer(cx - cameraX, cy, a);
        }
        ...
        flyingEnemies.forEach(check);
        necromancers.forEach(check);
        vortexes.forEach(check);
        homelessEnemies.forEach(check);
        
        if (boss && boss.active) check(boss);
        
        if (hit) {
            for (let i = 0; i < 25; i++) {
                particlePool.acquire(cx + (Math.random() - 0.5) * 40, cy + (Math.random() - 0.5) * 40, '#ffaa33');
            }
            shakeScreen(3);
        }
    }
    ...
    createHitEffect(x, y) {
        for (let i = 0; i < 15; i++) {
            particlePool.acquire(x + (Math.random() - 0.5) * 20, y + (Math.random() - 0.5) * 20, '#ff5500');
        }
    }
    
    createJumpParticles() {
        for (let i = 0; i < 12; i++) {
            particlePool.acquire(this.x + this.width / 2, this.y + this.height, this.color);
        }
    }
    ...
    checkLavaDamage() {
        for (let lp of lavaPlatforms) {
            if (this.x < lp.x + lp.width && this.x + this.width > lp.x && 
                this.y + this.height > lp.y - 10 && this.y < lp.y + lp.height) {
                
                if (Date.now() - (lp.lastDamage || 0) > 900) {
                    this.takeDamage(Math.floor(maxHealth * 0.25), 0, -5, '#ff4500');
                    lp.lastDamage = Date.now();
                    AudioSys.lavaBurn();
                    
                    for (let i = 0; i < 10; i++) {
                        particlePool.acquire(this.x + Math.random() * this.width, this.y + this.height, '#ff4500');
                    }
                }
            }
        }
    ...
    takeDamage(a, kx, ky, cl) {
        ...
        this.knockback(kx, ky);
        this.invulnerable = 45;
        AudioSys.hit();
        roundDamage += a;
        
        for (let i = 0; i < 15; i++) {
            particlePool.acquire(this.x + this.width / 2, this.y + this.height / 2, cl || '#ff2e63');
        }
    ...
    draw(ctx, cx) {
        ...
        for (let i = 0; i < this.trail.length; i++) {
            ...
            if (skin.type === 'sphere') {
                ctx.beginPath();
                ctx.arc(this.trail[i].x - cx + 20, this.trail[i].y + 20, 20, 0, Math.PI * 2);
                ctx.fill();
            } else {
                ctx.fillRect(this.trail[i].x - cx, this.trail[i].y, 40, 40);
            }
        }
        ...
        if (skin.type === 'sphere') {
            ctx.save();
            ctx.translate(dx + 20, this.y + 20);
            ctx.rotate(this.rotation);
            
            const g = ctx.createRadialGradient(-8, -8, 2, 0, 0, 20);
            g.addColorStop(0, skin.color);
            g.addColorStop(0.6, skin.color + '88');
            g.addColorStop(1, skin.color + '44');
            ...
            if (skin.texture) {
                ...
                if (img.complete) {
                    ctx.drawImage(img, dx, this.y, 40, 40);
                }
                ...
                if (skin.pattern === 'stripes') {
                    ...
                    for (let i = 0; i < 5; i++) {
                        ctx.beginPath();
                        ctx.moveTo(dx, this.y + i*10);
                        ctx.quadraticCurveTo(dx+20, this.y+i*10+5, dx+40, this.y+i*10);
                        ctx.stroke();
                    }
                } else if (skin.pattern === 'neon') {
                    ctx.shadowColor = skin.color;
                    ctx.shadowBlur = 15;
                    ctx.strokeRect(dx, this.y, 40, 40);
                    ctx.shadowBlur = 0;
                }
            }
            
            ctx.fillStyle = '#222';
            ctx.fillRect(dx + 10, this.y + 10, 8, 8);
            ctx.fillRect(dx + 22, this.y + 10, 8, 8);
        } else if (skin.type === 'pyramid') {
            ctx.fillStyle = skin.color;
            ctx.beginPath();
            ctx.moveTo(dx + 20, this.y);
            ctx.lineTo(dx + 40, this.y + 40);
            ctx.lineTo(dx, this.y + 40);
            ctx.fill();
            
            ctx.fillStyle = '#222';
            ctx.beginPath();
            ctx.arc(dx + 14, this.y + 22, 4, 0, Math.PI * 2);
            ctx.arc(dx + 26, this.y + 22, 4, 0, Math.PI * 2);
            ctx.fill();
            
            ctx.fillStyle = '#fff';
            ctx.beginPath();
            ctx.arc(dx + 15, this.y + 21, 2, 0, Math.PI * 2);
            ctx.arc(dx + 27, this.y + 21, 2, 0, Math.PI * 2);
            ctx.fill();
            ...
            ctx.fillRect(dx + 10, this.y + 10, 8, 8);
            ctx.fillRect(dx + 22, this.y + 10, 8, 8);
            ctx.fillRect(dx + 10, this.y + 25, 20, 4);
        }
        ...
        if (['magma', 'royal'].includes(skin.id) || skin.type === 'sphere') {
            ctx.shadowColor = skin.color;
            ctx.shadowBlur = 15;
        ...
        if (this.swingEffect > 0) {
            ctx.beginPath();
            ctx.arc(dx + 20, this.y + 20, this.meleeRadius, 0, Math.PI * 2);
        ...
        if (this.jumpCount >= 1 && !this.jumping && this.maxJumps > 1) {
            ...
            ctx.beginPath();
            ctx.arc(dx + 20, this.y - 8, 5, 0, Math.PI * 2);
            ctx.fill();
        }
    }
}
...
class Boss {
    constructor(x, y) {
        ...
        this.cd = 0;
        this.active = true;
        this.tex = bossTextures.getRandom(enCol);
        this.phase = 0;
        this.mp = 0;
        this.sp = 0;
        this.color = this.tex.type === 'color' ? this.tex.color : '#ff0000';
    }
    ...
    update() {
        ...
        if (Math.abs(this.x - player.x) > 200) {
            ...
            if (this.x < cameraX + 100 || this.x > cameraX + canvas.width - 140) {
                this.dir *= -1;
            }
        }
        
        this.y += Math.sin(this.mp) * 2;
        
        if (this.cd <= 0) {
            this.attack();
            this.cd = 120 - (currentLevel * 2);
        }
        
        if (player.checkCollision(this)) {
            if (player.takeDamage(30, this.x < player.x ? 15 : -15, -10, '#ff0000')) return;
        }
    }
    ...
    attack() {
        ...
        if (this.sp === 0) {
            for (let i = 0; i < 15; i++) {
                particlePool.acquire(this.x + 20, this.y + 20, '#ff0000');
            }
        } else if (this.sp === 1) {
            for (let i = -1; i <= 1; i++) {
                particlePool.acquire(this.x + 20, this.y + 20, 0, 4, 10, '#ff6600', false);
            }
        } else {
            for (let i = 0; i < 8; i++) {
                const a = (i / 8) * Math.PI * 2;
                particlePool.acquire(this.x + 20, this.y + 20, Math.cos(a) * 6, Math.sin(a) * 6, 8, '#ff0000');
            }
        }
    ...
    takeDamage(d = 1) {
        ...
        for (let i = 0; i < 10; i++) {
            particlePool.acquire(this.x + Math.random() * 40, this.y + Math.random() * 40, '#ff0000');
        }
    ...
    defeat() {
        ...
        for (let i = 0; i < 100; i++) {
            particlePool.acquire(this.x + 20, this.y + 20, this.color);
        }
        ...
        if (Math.random() < 0.8) {
            powerUps.push({ x: this.x + 20, y: this.y + 20, size: 20, color: '#FF2E63', type: 'health' });
        }
        
        if (Math.random() < 0.5) {
            powerUps.push({ x: this.x + 60, y: this.y + 20, size: 20, color: '#FFDE7D', type: 'dash' });
        }
    ...
    draw(ctx) {
        ...
        if (this.tex.type === 'image' && this.tex.image) {
            try {
                ctx.drawImage(this.tex.image, dx, this.y, 40, 40);
            } catch(e) {
                ctx.fillStyle = this.color;
                ctx.fillRect(dx, this.y, 40, 40);
            }
            ...
            for (let i = 0; i < 40; i += 20) {
                for (let j = 0; j < 40; j += 20) {
                    if ((i/20 + j/20) % 2 === 0) {
                        ctx.fillRect(dx + i, this.y + j, 20, 20);
                    }
                }
            }
        }
        ...
        ctx.fillStyle = '#fff';
        ctx.beginPath();
        ctx.arc(dx + 10, this.y + 15, 5, 0, Math.PI * 2);
        ctx.arc(dx + 30, this.y + 15, 5, 0, Math.PI * 2);
        ctx.fill();
        
        ctx.fillStyle = '#000';
        ctx.beginPath();
        ctx.arc(dx + 10 + (this.dir * 2), this.y + 15, 2, 0, Math.PI * 2);
        ctx.arc(dx + 30 + (this.dir * 2), this.y + 15, 2, 0, Math.PI * 2);
        ctx.fill();
        
        if (this.health < this.maxHealth * 0.3 && Math.floor(Date.now() / 200) % 2 === 0) {
            ctx.strokeStyle = '#ff0000';
            ctx.lineWidth = 4;
            ctx.strokeRect(dx - 5, this.y - 5, 50, 50);
        }
    }
}
...
class Enemy {
    constructor(x, y, t = 0) {
        ...
        this.height = 40;
        this.type = t;
        this.color = enCol[Math.floor(Math.random() * enCol.length)];
        this.speed = t === 0 ? 0 : 1 + Math.random() * 2;
        this.dir = Math.random() > 0.5 ? 1 : -1;
        this.sc = 0;
        this.pr = 100 + Math.random() * 200;
        this.sx = x;
        this.jc = 0;
        this.health = CONFIG.enemies.baseHealth[t === 0 ? 'shooter' : t === 1 ? 'patrol' : 'jumper'];
        this.maxHealth = this.health;
        this.scoreValue = CONFIG.enemies.baseScore[t === 0 ? 'shooter' : t === 1 ? 'patrol' : 'jumper'];
        this.tex = Math.floor(Math.random() * 3);
        this.active = true;
    }
    ...
    update() {
        ...
        if (this.type === 1) {
            this.x += this.speed * this.dir;
            if (Math.abs(this.x - this.sx) > this.pr) {
                this.dir *= -1;
            }
        }
        
        if (this.type === 2 && this.jc <= 0) {
            let on = false;
            for (let p of platforms) {
            ...
            if (on) {
                const d = player.x > this.x ? 1 : -1;
                this.y -= 12 + Math.random() * 4;
                this.x += d * (8 + Math.random() * 4);
                this.jc = 60 + Math.random() * 120;
            }
        }
        ...
        for (let p of platforms) {
            if (this.x < p.x + p.width && this.x + this.width > p.x && 
                ...
                if (this.type === 1 && (this.x <= p.x + 5 || this.x + this.width >= p.x + p.width - 5)) {
                    this.dir *= -1;
                }
            }
        }
    ...
    destroy(s) {
        ...
        if (s !== 'necro') {
            deadEnemiesForNecro.push({ x: this.x, y: this.y, color: this.color, timer: 180 });
        }
        ...
        if (Math.random() < 0.35) {
            coins.push({ x: this.x + 20, y: this.y + 20, size: 12, color: '#FFDE7D', bounce: 0 });
        }
        
        if (Math.random() < 0.12) {
            powerUps.push({ x: this.x + 20, y: this.y + 20, size: 15, color: '#FF2E63', type: 'health' });
        }
    }
    ...
    draw(ctx) {
        ...
        ctx.fillStyle = this.color;
        ctx.fillRect(this.x - cameraX, this.y, 40, 40);
        
        ctx.fillStyle = 'rgba(0,0,0,0.25)';
        
        if (this.tex === 0) {
            for (let i = 0; i < 40; i += 15) {
                ctx.fillRect(this.x - cameraX + i, this.y, 8, 40);
            }
        } else if (this.tex === 1) {
            ...
            ctx.beginPath();
            ctx.arc(this.x - cameraX + 20, this.y + 20, 10, 0, Math.PI * 2);
            ctx.fill();
        }
        ...
        if (this.health < this.maxHealth) {
            ...
            ctx.fillRect(this.x - cameraX, this.y - 8, (40 * this.health) / this.maxHealth, 4);
        }
    }
}
...
class FlyingEnemy {
    constructor(x, y) {
        ...
        this.width = 40;
        this.height = 40;
        this.color = flyCol[Math.floor(Math.random() * flyCol.length)];
        this.speed = 1.5 + Math.random() * 1.5;
        this.dir = Math.random() > 0.5 ? 1 : -1;
        this.health = 2;
        this.maxHealth = 2;
        this.wp = 0;
        this.fa = 20 + Math.random() * 30;
        this.fs = 0.05 + Math.random() * 0.03;
    ...
    update() {
        ...
        this.wp += 0.2;
        this.fp += this.fs;
        this.y = this.oy + Math.sin(this.fp) * this.fa;
        this.x += this.speed * this.dir;
    ...
    draw(ctx) {
        ...
        ctx.beginPath();
        ctx.moveTo(this.x - cameraX + 55, wy);
        ctx.quadraticCurveTo(this.x - cameraX + 65, wy - wa, this.x - cameraX + 55, wy);
        ctx.fill();
        
        ctx.fillStyle = '#000';
        ctx.fillRect(this.x - cameraX + 8, this.y + 8, 6, 6);
        ctx.fillRect(this.x - cameraX + 26, this.y + 8, 6, 6);
...
class NecromancerEnemy {
    ...
    update() {
        ...
        if (this.sc <= 0) {
            for (let d of deadEnemiesForNecro) {
                if (d.timer > 0 && Math.hypot(d.x - this.x, d.y - this.y) < 200) {
                    d.timer--;
                    
                    if (d.timer <= 0) {
                        const n = new Enemy(d.x, d.y, 0);
                        enemies.push(n);
                        deadEnemiesForNecro = deadEnemiesForNecro.filter(x => x !== d);
                        AudioSys.necroSummon();
                        
                        for (let i = 0; i < 20; i++) {
                            particlePool.acquire(d.x, d.y, '#8e44ad');
                        }
                        break;
                    }
                }
    ...
    destroy() {
        this.active = false;
        addScore(300 * comboMultiplier);
        updateCombo();
        AudioSys.collect();
        
        for (let i = 0; i < 30; i++) {
            particlePool.acquire(this.x + 20, this.y + 20, '#8e44ad');
        }
    }
    ...
    draw(ctx) {
        ...
        ctx.beginPath();
        ctx.moveTo(dx + 20, this.y - 15);
        ctx.lineTo(dx - 5, this.y + 10);
        ctx.lineTo(dx + 45, this.y + 10);
        ctx.fill();
        
        ctx.fillStyle = '#ff00ff';
        ctx.beginPath();
        ctx.arc(dx + 12, this.y + 15, 4, 0, Math.PI * 2);
        ctx.arc(dx + 28, this.y + 15, 4, 0, Math.PI * 2);
        ctx.fill();
        ...
        if (this.sc < 60) {
            ...
            ctx.lineWidth = 2;
            ctx.beginPath();
            ctx.arc(dx + 20, this.y + 20, 40 + Math.sin(Date.now() / 100) * 10, 0, Math.PI * 2);
            ctx.stroke();
        }
        
        if (this.health < this.maxHealth) {
            ctx.fillStyle = '#4af626';
            ctx.fillRect(dx, this.y - 12, (40 * this.health) / this.maxHealth, 4);
        }
        
        ctx.fillStyle = '#fff';
        ctx.font = 'bold 11px Unbounded';
        ctx.textAlign = 'center';
        ctx.fillText('НЕКРОМАНТ', dx + 20, this.y - 20);
    }
}
...
class VortexEnemy {
    ...
    update() {
        ...
        this.rot += 0.1;
        this.fp += 0.03;
        this.y = this.oy + Math.sin(this.fp) * 15;
        ...
        if (this.shootCooldown <= 0 && Math.abs(this.x - player.x) < 400 && Math.abs(this.y - player.y) < 300) {
            ...
            for (let i = 0; i < 5; i++) {
                particlePool.acquire(this.x + 20, this.y + 20, '#00ffff');
            }
        }
        ...
        if (d < 250 && d > 30) {
            player.velX += (dx / d) * 0.8;
            player.velY += (dy / d) * 0.4;
        }
    ...
    draw(ctx) {
        ...
        for (let i = 0; i < 4; i++) {
            ctx.fillStyle = `rgba(0,255,255,${0.3 + i * 0.15})`;
            ctx.beginPath();
            ctx.arc(0, 0, 20 - i * 5, i * 0.5, i * 0.5 + Math.PI);
            ctx.fill();
        }
        ...
        ctx.fillStyle = '#fff';
        ctx.beginPath();
        ctx.arc(dx + 13, this.y + 15, 5, 0, Math.PI * 2);
        ctx.arc(dx + 27, this.y + 15, 5, 0, Math.PI * 2);
        ctx.fill();
        
        ctx.fillStyle = '#000';
        ctx.beginPath();
        ctx.arc(dx + 14, this.y + 16, 2, 0, Math.PI * 2);
        ctx.arc(dx + 28, this.y + 16, 2, 0, Math.PI * 2);
        ctx.fill();
        
        ctx.fillStyle = '#fff';
        ctx.beginPath();
        ctx.arc(dx + 20, this.y + 20, 6, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.font = 'bold 11px Unbounded';
        ctx.textAlign = 'center';
        ctx.fillText('ВИХРЬ', dx + 20, this.y - 20);
    }
}
...
class HomelessEnemy {
    ...
    update() {
        ...
        this.bw += 0.15;
        this.y = this.plat.y - this.height;
        this.x = this.plat.x + this.plat.width / 2 - 20;
        
        this.lavaCooldown--;
        
        if (this.lavaCooldown <= 0 && Math.abs(this.x - player.x) < 400) {
            AudioSys.homelessShoot();
            
            for (let i = 0; i < 8; i++) {
                const angle = (i / 8) * Math.PI * 2 - Math.PI / 2;
                lavaProjectiles.push(new LavaProjectile(this.x + 20, this.y + 10, angle));
            }
            
            this.lavaCooldown = this.lavaInterval;
            
            for (let i = 0; i < 15; i++) {
                particlePool.acquire(this.x + 20, this.y + 10, '#ff4500');
            }
        }
    }
    
    takeDamage(d = 1, s) {
        this.health -= (s === 'arrow' ? d * 1.67 : d);
        if (this.health <= 0) {
            this.destroy();
            return true;
        }
    ...
    destroy() {
        this.active = false;
        addScore(180 * comboMultiplier);
        updateCombo();
        AudioSys.collect();
        
        for (let i = 0; i < 25; i++) {
            particlePool.acquire(this.x + 20, this.y + 20, '#8B7355');
        }
    ...
    draw(ctx) {
        ...
        ctx.fillStyle = this.color;
        ctx.fillRect(dx, this.y + 5, 40, 35);
        
        ctx.fillStyle = '#D2B48C';
        ctx.beginPath();
        ctx.arc(dx + 20, this.y + 10, 12, 0, Math.PI * 2);
        ctx.fill();
        
        ctx.fillStyle = '#6B5443';
        ctx.beginPath();
        ctx.moveTo(dx + 8, this.y + 12);
        
        for (let i = 0; i < 6; i++) {
            ctx.quadraticCurveTo(dx + 12 + i * 4, this.y + 20 + Math.sin(this.bw + i) * 3, dx + 14 + i * 4, this.y + 12);
        }
        
        ctx.lineTo(dx + 32, this.y + 12);
        ctx.fill();
        
        ctx.fillStyle = '#000';
        ctx.beginPath();
        ctx.arc(dx + 15, this.y + 8, 3, 0, Math.PI * 2);
        ctx.arc(dx + 25, this.y + 8, 3, 0, Math.PI * 2);
        ctx.fill();
        
        ctx.fillStyle = '#4a3728';
        ctx.fillRect(dx + 5, this.y - 2, 30, 8);
        ctx.fillRect(dx + 2, this.y - 6, 36, 4);
        
        if (this.health < this.maxHealth) {
            ctx.fillStyle = '#4af626';
            ctx.fillRect(dx, this.y - 15, (40 * this.health) / this.maxHealth, 4);
        }
        
        ctx.fillStyle = '#fff';
        ctx.font = 'bold 11px Unbounded';
        ctx.textAlign = 'center';
        ctx.fillText('БОМЖ', dx + 20, this.y - 22);
        
        if (this.lavaCooldown > 0) {
            ctx.fillStyle = `rgba(255,69,0,${0.3 + 0.4 * Math.sin(Date.now() / 100)})`;
            ctx.beginPath();
            ctx.arc(dx + 20, this.y - 25, 8 + Math.sin(Date.now() / 150) * 2, 0, Math.PI * 2);
            ctx.fill();
        }
    }
}
...
class Platform {
    ...
    update() {
        if (this.glow) this.gp += 0.05;
        
        if (this.type === 'moving' && !this.broken) {
            this.x += this.ms * this.md;
            if (Math.abs(this.x - this.ox) > 100) {
                this.md *= -1;
            }
        }
        
        if (this.type === 'breaking' && this.bt > 0) {
            this.bt--;
            if (this.bt === 0) {
                this.broken = true;
    ...
    draw(ctx) {
        ...
        if (this.isLava) {
            ...
            ctx.fillRect(this.x - cameraX, this.y + 5, this.width, this.height - 5);
            
            ctx.fillStyle = '#ff6600';
            for (let i = 0; i < this.width; i += 15) {
                ctx.beginPath();
                ctx.arc(this.x - cameraX + i + 7, this.y + 10, 4, 0, Math.PI);
                ctx.fill();
            }
        } else {
            ctx.fillStyle = this.tex.c;
            ...
            if (p === 'stripes') {
                for (let i = 0; i < this.width; i += 30) {
                    ctx.fillRect(this.x - cameraX + i, this.y, 15, this.height);
                }
            } else if (p === 'dots') {
                ...
                for (let i = 0; i < this.width; i += 8) {
                    for (let j = 0; j < this.height; j += 8) {
                        if ((Math.floor(i/8) + Math.floor(j/8)) % 2 === 0) {
                            ctx.fillRect(this.x - cameraX + i, this.y + j, 8, 8);
                        }
                    }
                }
            } else if (p === 'zigzag') {
                for (let i = 0; i < this.width; i += 20) {
                    ...
                    ctx.lineTo(this.x - cameraX + i + 10, this.y);
                    ctx.lineTo(this.x - cameraX + i + 20, this.y + this.height);
                    ctx.fill();
                }
            } else if (p === 'bricks') {
                for (let i = 0; i < this.width; i += 25) {
                    for (let j = 0; j < this.height; j += 12) {
                        if ((i/25 + j/12) % 2 === 0) {
                            ctx.fillRect(this.x - cameraX + i, this.y + j, 25, 12);
                        }
                    }
                }
            } else if (p === 'waves') {
                for (let i = 0; i < this.width; i += 30) {
                    ...
                    ctx.lineTo(this.x - cameraX + i + 30, this.y + this.height);
        ...
        if (this.glow && !this.isLava) {
            const gi = Math.sin(this.gp) * 0.3 + 0.7;
            ctx.shadowColor = this.tex.c;
            ctx.shadowBlur = 15 * gi;
            ctx.strokeStyle = this.tex.c;
            ctx.lineWidth = 2;
        ...
        if (this.type === 'moving') {
            ...
            for (let i = 0; i < ac; i++) {
                const ax = this.x - cameraX + 15 + i * 30;
                ctx.beginPath();
                ctx.moveTo(ax, this.y + this.height / 2);
                ctx.lineTo(ax + 10 * this.md, this.y + 10);
                ctx.lineTo(ax + 10 * this.md, this.y + this.height - 10);
                ctx.closePath();
                ctx.fill();
            }
        }
...
function generateLevel(level) {
    ...
    let lx = 100;
    let ly = canvas.height - 250;
    let dir = 1;
    
    for (let i = 0; i < pc; i++) {
        const w = 70 + Math.random() * 130;
        const mig = 120 + (level * 10);
        const mag = 220 + (level * 15);
        ...
        if (i > 2 && i < pc - 2 && Math.random() < 0.4) {
            const cc = Math.floor(Math.random() * 3) + 1;
            for (let j = 0; j < cc; j++) {
                coins.push({ x: x + 20 + j * 25, y: y - 30, size: 12, color: '#FFDE7D', bounce: Math.random() * Math.PI * 2 });
            }
        }
    ...
    if (level >= 3) {
        const safePlat = platforms[Math.floor(Math.random() * (platforms.length - 5)) + 3];
        const lp = new Platform(safePlat.x - 30, safePlat.y, 110, Math.floor(Math.random() * platTex.length), true);
        platforms.push(lp);
        lavaPlatforms.push(lp);
        homelessEnemies.push(new HomelessEnemy(safePlat.x + 15, safePlat.y - 40, lp));
    }
    
    const newTypes = ['necromancer', 'vortex'];
    
    for (let i = 0; i < ec; i++) {
        const pi = Math.floor(Math.random() * (platforms.length - 5)) + 3;
        ...
        if (level >= 3 && i > 0 && i % 4 === 0) {
            ...
            if (t === 'necromancer') {
                necromancers.push(new NecromancerEnemy(p.x + p.width / 2 - 20, p.y - 40));
            } else {
                vortexes.push(new VortexEnemy(p.x + p.width / 2 - 20, 150 + Math.random() * 200));
            }
        } else {
    ...
    for (let i = 0; i < fc; i++) {
        flyingEnemies.push(new FlyingEnemy(300 + Math.random() * (levelWidth - 600), 100 + Math.random() * 200));
    }
    ...
    for (let cx = 800; cx < levelWidth - 300; cx += 800) {
        platforms.push(new Platform(cx, canvas.height - 180, 100, 2));
    }
...
function updateCamera() {
    const t = player.x - canvas.width * CONFIG.camera.playerOffset;
    cameraX += (t - cameraX) * CONFIG.camera.followSpeed;
...
function updateCoins() {
    for (let i = coins.length - 1; i >= 0; i--) {
        const c = coins[i];
        c.bounce += 0.1;
        c.y += Math.sin(c.bounce) * 0.5;
        
        if (player.checkCollision({ x: c.x - c.size / 2, y: c.y - c.size / 2, width: c.size, height: c.size })) {
            ...
            for (let j = 0; j < 15; j++) {
                particlePool.acquire(c.x, c.y, '#FFDE7D');
            }
...
function updateKeys() {
    for (let i = levelKeys.length - 1; i >= 0; i--) {
        ...
        k.floatOffset += 0.06;
        const ky = k.y + Math.sin(k.floatOffset) * 6;
        
        if (player.checkCollision({ x: k.x, y: ky, width: k.size, height: k.size })) {
            k.collected = true;
            totalKeys++;
            saveAllData();
            AudioSys.collect();
            ...
            for (let j = 0; j < 20; j++) {
                particlePool.acquire(k.x + k.size / 2, ky + k.size / 2, '#00BFFF');
            }
            
            levelKeys.splice(i, 1);
        }
    }
}
...
function updatePowerUps() {
    for (let i = powerUps.length - 1; i >= 0; i--) {
        const p = powerUps[i];
        p.y += Math.sin(Date.now() / 500) * 0.5;
        
        if (player.checkCollision({ x: p.x - p.size / 2, y: p.y - p.size / 2, width: p.size, height: p.size })) {
            if (p.type === 'health') {
                playerHealth = Math.min(maxHealth, playerHealth + 30);
                updateHealthBar();
            } else if (p.type === 'dash') {
                player.dashCharges = Math.min(CONFIG.player.maxDashes, player.dashCharges + 1);
                updateDashIndicator();
            }
            
            for (let j = 0; j < 20; j++) {
                particlePool.acquire(p.x, p.y, p.color);
            }
...
function updateWindProjectiles() {
    for (let i = windProjectiles.length - 1; i >= 0; i--) {
        windProjectiles[i].update();
        if (!windProjectiles[i].active) windProjectiles.splice(i, 1);
    }
}

function updateLavaProjectiles() {
    for (let i = lavaProjectiles.length - 1; i >= 0; i--) {
        lavaProjectiles[i].update();
        if (!lavaProjectiles[i].active) lavaProjectiles.splice(i, 1);
    }
}

function updateNewEnemies() {
    necromancers.forEach(e => e.update());
    vortexes.forEach(e => e.update());
    homelessEnemies.forEach(e => e.update());
    
    deadEnemiesForNecro = deadEnemiesForNecro.filter(d => {
        d.timer--;
        return d.timer > 0;
    });
}
...
function checkEnemyCollisions() {
    ...
    for (let e of necromancers) {
        if (e.active && player.checkCollision(e)) {
            if (player.takeDamage(25, e.x < player.x ? 10 : -10, -8, '#8e44ad')) return;
        }
    }
    
    for (let e of vortexes) {
        if (e.active && player.checkCollision(e)) {
            if (player.takeDamage(30, 0, -12, '#00ffff')) return;
        }
    }
    
    for (let e of homelessEnemies) {
        if (e.active && player.checkCollision(e)) {
            if (player.takeDamage(22, e.x < player.x ? 8 : -8, -6, '#8B7355')) return;
        }
    }
}

function checkCheckpoints() {
    if (player.x > lastCheckpointX + 800) {
...
function drawBackground() {
    ...
    for (let l = 0; l < 3; l++) {
        ...
        for (let i = 0; i < 40; i++) {
            const x = (i * 67 + cameraX * s) % canvas.width;
            const y = (i * 41 + l * 100) % canvas.height;
...
function drawGoal() {
    const gx = levelWidth - cameraX;
    
    if (gx < canvas.width + 100 && gx > -100) {
        const g = ctx.createRadialGradient(gx, canvas.height / 2, 10, gx, canvas.height / 2, 180);
        g.addColorStop(0, '#4af626aa');
        g.addColorStop(1, '#4af62600');
        
        ctx.fillStyle = g;
        ctx.fillRect(gx - 180, 0, 360, canvas.height);
        ...
        const p = Math.sin(Date.now() / 250) * 0.4 + 0.6;
        ctx.fillStyle = `rgba(74,246,38,${p})`;
        ctx.font = 'bold 26px monospace';
        ctx.textAlign = 'center';
        ctx.fillText('🏁 ФИНИШ', gx, canvas.height - 45);
    }
}
...
function drawParticles() {
    ...
    for (let p of particlePool.activeObjects) {
        p.update();
        p.draw(ctx, cameraX);
        if (!p.active) particlePool.release(p);
    }
}
...
function drawCoins() {
    coins.forEach(c => {
        ...
        ctx.beginPath();
        ctx.arc(c.x - cameraX, c.y, c.size, 0, Math.PI * 2);
        ctx.fill();
        
        ctx.fillStyle = '#FFFFFF88';
        ctx.beginPath();
        ctx.arc(c.x - cameraX - 3, c.y - 3, c.size / 3, 0, Math.PI * 2);
        ctx.fill();
    });
}
...
function drawKeys() {
    levelKeys.forEach(k => {
        ...
        ctx.shadowBlur = 12;
        ctx.fillText('🔑', k.x + k.size / 2 - cameraX, ky + k.size / 2);
        ctx.shadowBlur = 0;
    });
}
...
function drawPowerUps() {
    powerUps.forEach(p => {
        ...
        if (p.type === 'health') {
            ctx.moveTo(p.x - cameraX, p.y + p.size / 2);
            ctx.bezierCurveTo(p.x - cameraX, p.y, p.x - cameraX - p.size, p.y, p.x - cameraX - p.size, p.y + p.size / 2);
            ctx.bezierCurveTo(p.x - cameraX - p.size, p.y + p.size, p.x - cameraX, p.y + p.size * 1.5, p.x - cameraX, p.y + p.size * 1.5);
            ctx.bezierCurveTo(p.x - cameraX, p.y + p.size * 1.5, p.x - cameraX + p.size, p.y + p.size, p.x - cameraX + p.size, p.y + p.size / 2);
            ctx.bezierCurveTo(p.x - cameraX + p.size, p.y, p.x - cameraX, p.y, p.x - cameraX, p.y + p.size / 2);
        } else {
...
function drawWindProjectiles() {
    windProjectiles.forEach(w => w.draw(ctx));
}

function drawLavaProjectiles() {
    lavaProjectiles.forEach(l => l.draw(ctx));
}
...
function gameLoop() {
    ...
    updatePowerUps();
    updateArrows();
    updateWindProjectiles();
    updateLavaProjectiles();
    updateNewEnemies();
    decayCombo();
    ...
    necromancers.forEach(e => e.draw(ctx));
    vortexes.forEach(e => e.draw(ctx));
    homelessEnemies.forEach(e => e.draw(ctx));
...
function completeLevel() {
    ...
    const el = document.getElementById('eloChangeDisplay');
    el.textContent = (er.change >= 0 ? '+' : '') + er.change;
    el.style.color = er.change >= 0 ? '#4af626' : '#ff2e63';
    
    document.getElementById('levelComplete').style.display = 'flex';
    showEloChange(er.change);
    
    for (let i = 0; i < 80; i++) {
        particlePool.acquire(player.x + 20, player.y + 20, platTex[Math.floor(Math.random() * platTex.length)].c);
    }
...
function restartGame() {
    ...
    currentLevel = 1;
    score = 0;
    playerHealth = getCurrentClass().health || 100;
    ...
    if (isT || window.innerWidth <= 768) {
        ...
        if (document.getElementById('btnShoot')) {
            document.getElementById('btnShoot').addEventListener('touchstart', e => {
                e.preventDefault();
                if (equippedClass === 'archer' && arrowCooldown <= 0) {
                    player.shootArrow();
                    arrowCooldown = 25;
                }
            });
...
document.getElementById('particlesToggle').addEventListener('change', e => {
    ...
    if (!e.target.checked) particlePool?.releaseAll();
});
...
async function initGame() {
    ...
    const pi = setInterval(() => {
        pr += Math.random() * 15;
        document.getElementById('progressFill').style.width = `${Math.min(pr, 100)}%`;
