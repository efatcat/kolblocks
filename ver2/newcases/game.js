...
class Player {
    ...
    draw(ctx, cameraX) {
        ...
        if (skin.shape && skin.shape !== 'square') {
            ...
            switch(skin.shape) {
                ...
                case 'triangle':
                    ...
                    ctx.lineTo(centerX - this.width/2, centerY + this.height/2);
                    ctx.lineTo(centerX + this.width/2, centerY + this.height/2);
                    ctx.closePath();
                    ctx.fill();
                    break;
                ...
                case 'star':
                    ...
                    break;
                case 'hexagon':
                    this.drawHexagon(ctx, centerX, centerY, this.width/2);
                    ctx.fill();
                    break;
                case 'octopus':
                    this.drawOctopus(ctx, centerX, centerY, this.width/2);
                    ctx.fill();
                    break;
                default:
        ...
        if (this.swingEffect > 0) {
            ctx.beginPath();
            ctx.arc(centerX, centerY, CONFIG.melee.radius, 0, Math.PI * 2);
            ctx.fillStyle = `rgba(255, 80, 40, ${0.3 * (this.swingEffect / 8)})`;
            ctx.fill();
            ctx.beginPath();
            ctx.arc(centerX, centerY, CONFIG.melee.radius - 10, 0, Math.PI * 2);
        ...
        if (this.jumpCount === 1 && !this.jumping) {
            ...
            ctx.beginPath();
            ctx.arc(centerX, drawY - 8, 5, 0, Math.PI*2);
            ctx.fill();
        }
    }
