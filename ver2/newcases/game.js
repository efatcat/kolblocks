...
const AudioSys = {
    ...
    jump() { this.play(440, 0.15, 'sine', 0.15); }, meleeSwing() { this.play(380, 0.12, 'sawtooth', 0.2); },
    hit() { this.play(220, 0.2, 'sawtooth', 0.2, 0.3); }, collect() { this.play(660, 0.1, 'sine', 0.12); },
    combo() { this.play(523, 0.12, 'sine', 0.15); }, dash() { this.play(330, 0.1, 'triangle', 0.1); },
    checkpoint() { this.play(784, 0.3, 'sine', 0.2); }, levelComplete() { [523,659,784,1047].forEach((f,i)=>setTimeout(()=>this.play(f,0.2,'sine',0.15),i*100)); },
    bossSpawn() { [200,250,300,250,200].forEach((f,i)=>setTimeout(()=>this.play(f,0.3,'sawtooth',0.2,0.5),i*100)); },
    bossHit() { this.play(150, 0.3, 'sawtooth', 0.25, 0.4); }, bossDefeat() { [400,500,600,800,1000].forEach((f,i)=>setTimeout(()=>this.play(f,0.4,'square',0.2),i*150)); },
    gameOver() { [392,349,294,262].forEach((f,i)=>setTimeout(()=>this.play(f,0.3,'sawtooth',0.15,0.5),i*150)); },
    eloGain() { this.play(880, 0.2, 'sine', 0.2); }, eloLoss() { this.play(200, 0.25, 'sawtooth', 0.15); },
    chestOpen() { [300,450,600,900].forEach((f,i)=>setTimeout(()=>this.play(f,0.15,'square',0.15),i*80)); },
    arrowShoot() { this.play(550, 0.08, 'triangle', 0.15); }, arrowHit() { this.play(280, 0.15, 'sawtooth', 0.18, 0.4); },
    lavaBurn() { this.play(180, 0.3, 'sawtooth', 0.1, 0.2); }, necroSummon() { this.play(120, 0.4, 'sine', 0.2, 0.6); },
    windShoot() { this.play(480, 0.1, 'triangle', 0.12); }, windHit() { this.play(320, 0.15, 'sawtooth', 0.15, 0.3); }
};
...
class Particle {
    ...
    draw(ctx, cx) { if (!this.active) return; ctx.globalAlpha = this.life / this.maxLife; ctx.fillStyle = this.color; ctx.beginPath(); ctx.arc(this.x - cx, this.y, this.size, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = 1; }
}
class ObjectPool {
    constructor(createFn, maxSize = 500) { this.createFn = createFn; this.maxSize = maxSize; this.pool = []; this.active = []; }
    acquire(...args) { let o = this.pool.pop(); if (!o) o = this.createFn(...args); o.active = true; this.active.push(o); return o; }
    release(o) { o.active = false; const i = this.active.indexOf(o); if (i > -1) this.active.splice(i, 1); if (this.pool.length < this.maxSize) this.pool.push(o); }
...
// ==================== СНАРЯДЫ ====================
class Arrow {
    constructor(x, y, dir, dmg = 1) { this.x = x; this.y = y; this.w = 20; this.h = 4; this.spd = 15; this.dir = dir; this.dmg = dmg; this.active = true; this.life = 120; this.color = '#27ae60'; }
    update() {
        if (!this.active) return; this.x += this.spd * this.dir; this.life--;
        ...
        for (let e of targets) {
            ...
            if (this.x < e.x + e.width && this.x + this.w > e.x && this.y < e.y + e.height && this.y + this.h > e.y) {
                ...
                this.active = false; AudioSys.arrowHit(); for (let i = 0; i < 8; i++) particlePool.acquire(this.x, this.y, '#27ae60'); return;
            }
        }
    ...
    draw(ctx) {
        if (!this.active) return; ctx.save(); ctx.translate(this.x - cameraX + this.w / 2, this.y + this.h / 2); ctx.rotate(this.dir > 0 ? 0 : Math.PI);
        ctx.fillStyle = this.color; ctx.fillRect(-this.w / 2, -this.h / 2, this.w, this.h); ctx.fillStyle = '#1a5233';
        ctx.beginPath(); ctx.moveTo(this.w / 2, 0); ctx.lineTo(this.w / 2 + 8, -6); ctx.lineTo(this.w / 2 + 8, 6); ctx.fill(); ctx.restore();
    }
}
...
class WindProjectile {
    ...
    update() {
        if (!this.active) return; this.x += this.vx; this.y += this.vy; this.rotation += 0.2; this.life--;
    ...
    draw(ctx) {
        if (!this.active) return; ctx.save(); ctx.translate(this.x - cameraX + this.w / 2, this.y + this.h / 2); ctx.rotate(this.rotation);
        ctx.fillStyle = 'rgba(0, 255, 255, 0.8)'; ctx.beginPath(); ctx.arc(0, 0, this.w / 2, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = 'rgba(255, 255, 255, 0.6)'; ctx.beginPath(); ctx.arc(-2, -2, 3, 0, Math.PI * 2); ctx.fill(); ctx.restore();
    }
}
...
const bossTextures = {
    ...
    load() { return new Promise(res => { let c = 0; this.urls.forEach((u, i) => { let img = new Image(); img.crossOrigin = "anonymous"; img.onload = img.onerror = () => { c++; if (c === this.urls.length) { this.loaded = true; res(); } }; img.src = u; this.images[i] = img; }); }); },
    getRandom(colors) { return Math.random() < 0.7 ? { type: 'color', color: colors[Math.floor(Math.random() * colors.length)] } : { type: 'image', image: this.images[Math.floor(Math.random() * this.images.length)] }; }
};
...
const enCol = ['#FF2E63', '#FFDE7D', '#6A2C70', '#08D9D6', '#AA00FF'], flyCol = ['#FF00FF', '#00FFFF', '#FFFF00', '#FF6600'];
...
function updateUI() { document.getElementById('levelDisplay').textContent = currentLevel; document.getElementById('scoreDisplay').textContent = score; document.getElementById('comboCount').textContent = `x${comboCount}`; document.getElementById('eloValue').textContent = Math.floor(playerELO); document.getElementById('classDisplay').textContent = 'Класс: ' + PLAYER_CLASSES.find(c => c.id === equippedClass).name; }
function addScore(p) { score += Math.floor(p * comboMultiplier); updateUI(); }
function updateCombo() { comboCount++; comboTimer = 180; comboMultiplier = 1 + (comboCount - 1) * 0.1; maxCombo = Math.max(maxCombo, comboCount); const cd = document.getElementById('comboDisplay'); cd.classList.add('active'); document.getElementById('comboValue').textContent = comboCount; setTimeout(() => cd.classList.remove('active'), 1000); if (comboCount >= 5) AudioSys.combo(); }
...
function applyClassStats() { const c = getCurrentClass(); if (!c) return; maxHealth = c.health || 100; playerHealth = Math.min(playerHealth, maxHealth); player.speed = c.speed || 6; player.jumpPower = c.jumpPower || 16; player.gravity = c.gravity || 0.8; player.meleeDamage = c.meleeDamage || 1; player.maxJumps = c.maxJumps || 2; player.maxDashes = c.maxDashes || 2; player.meleeRadius = c.meleeRadius || CONFIG.melee.radius; updateHealthBar(); updateDashIndicator(); }

// ==================== СИСТЕМА ELO ====================
function updateEloDisplay() { document.getElementById('eloValue').textContent = Math.floor(playerELO); }
function showEloChange(ch) { const el = document.createElement('div'); el.className = 'elo-change ' + (ch >= 0 ? 'positive' : 'negative'); el.textContent = (ch >= 0 ? '+' : '') + Math.floor(ch); document.body.appendChild(el); if (ch >= 0) AudioSys.eloGain(); else AudioSys.eloLoss(); setTimeout(() => el.remove(), 1500); }
function calculateEloChange() { const c = Math.floor(roundCoins * CONFIG.elo.coinsMultiplier), d = Math.floor(roundDamage / CONFIG.elo.damageDivider), ch = c - d; playerELO = Math.max(0, playerELO + ch); saveAllData(); return { change: ch }; }
...
function saveAllData() { localStorage.setItem('kolblocks_elo', playerELO); localStorage.setItem('kolblocks_keys', totalKeys); localStorage.setItem('kolblocks_skins', JSON.stringify(unlockedSkins)); localStorage.setItem('kolblocks_auras', JSON.stringify(unlockedAuras)); localStorage.setItem('kolblocks_classes', JSON.stringify(unlockedClasses)); localStorage.setItem('kolblocks_equipped', equippedSkin); localStorage.setItem('kolblocks_equipped_aura', equippedAura || ''); localStorage.setItem('kolblocks_equipped_class', equippedClass); }

function openShop() { gameRunning = false; document.getElementById('pauseMenu').style.display = 'none'; document.getElementById('caseShopScreen').style.display = 'flex'; document.getElementById('shopKeyCount').textContent = totalKeys; renderChestTypes(); renderSkins(); renderAuras(); renderClasses(); }
...
function renderChestTypes() {
    ...
    CHEST_TYPES.forEach(ch => {
        ...
        // 👇 Передаём this (кнопку) явно, чтобы избежать потери ссылки
...
function openChestType(id, btn) {
    ...
    setTimeout(() => {
        ...
        if (has) { totalKeys += Math.floor(ch.price * 0.2); saveAllData(); document.getElementById('shopKeyCount').textContent = totalKeys; showResult('Дубликат: ' + dr.name, 'Возврат: +' + Math.floor(ch.price * 0.2) + ' 🔑', '#aaa'); }
...
function buyClass(id, price) { if (totalKeys < price) return; if (confirm(`Купить ${getClassData(id).name} за ${price} 🔑?`)) { totalKeys -= price; unlockedClasses.push(id); equippedClass = id; saveAllData(); applyClassStats(); renderClasses(); document.getElementById('shopKeyCount').textContent = totalKeys; showResult('Класс получен!', getClassData(id).name, getClassData(id).color); } }
...
function openAuraChest() { if (totalKeys < 12) { alert('Нужно минимум 12 🔑!'); return; } totalKeys -= 12; saveAllData(); document.getElementById('shopKeyCount').textContent = totalKeys; const btn = document.getElementById('openAuraChestBtn'), chest = document.getElementById('auraChestVisual'); if (btn) { btn.disabled = true; chest.classList.add('shaking'); } AudioSys.chestOpen(); setTimeout(() => { chest.classList.remove('shaking'); let r = Math.random() * 100, cm = 0, dr = AURA_SKINS[0]; for (let a of AURA_SKINS) { cm += a.chance; if (r < cm) { dr = a; break; } } const has = unlockedAuras.includes(dr.id); if (has) { totalKeys += 3; saveAllData(); document.getElementById('shopKeyCount').textContent = totalKeys; showResult('Дубликат ауры: ' + dr.name, 'Возврат: +3 🔑', '#aaa'); } else { unlockedAuras.push(dr.id); saveAllData(); showResult('Новая аура: ' + dr.name + '!', dr.color); } if (btn) btn.disabled = false; renderAuras(); }, 1200); }
function showResult(t, tx, cl) { document.getElementById('resultTitle').textContent = t; document.getElementById('resultText').textContent = tx; if (cl) document.getElementById('chestResult').querySelector('h3').style.color = cl; document.getElementById('chestResult').style.display = 'block'; }
...
function loadBatidaoImage() { const img = new Image(); img.crossOrigin = "Anonymous"; img.onload = () => { batidaoImage = img; }; img.onerror = () => { batidaoImage = null; }; img.src = 'https://images.genius.com/3849b06fe11fa1c89ba96465b298457c.1000x1000x1.png'; }
...
class Player {
    ...
    reset() {
        ...
        this.prevY = this.y; // Сохраняем позицию для коллизии
        return this;
    }
    update(k) {
        this.prevY = this.y; // Запоминаем позицию прошлого кадра
    ...
    meleeAttack() {
        ...
        let hit = false;
        const check = (e) => { if (!e.active) return; const d = Math.hypot(cx - (e.x + e.width / 2), cy - (e.y + e.height / 2)); if (d < this.meleeRadius) { if (e.takeDamage(this.meleeDamage)) { if (e instanceof Enemy) enemies = enemies.filter(x => x !== e); else if (e instanceof FlyingEnemy) flyingEnemies = flyingEnemies.filter(x => x !== e); else if (e instanceof HomelessEnemy) homelessEnemies = homelessEnemies.filter(x => x !== e); else if (e instanceof NecromancerEnemy) necromancers = necromancers.filter(x => x !== e); else if (e instanceof VortexEnemy) vortexes = vortexes.filter(x => x !== e); else if (e === boss) { boss = null; document.getElementById('bossHealthBar').style.display = 'none'; } } hit = true; updateCombo(); this.createHitEffect(e.x + e.width / 2, e.y + e.height / 2); } };
    ...
    draw(ctx, cx) {
        ...
        else if (skin.type === 'decorated') { ctx.fillStyle = skin.color; ctx.fillRect(dx, this.y, 40, 40); ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; if (skin.pattern === 'stripes') { for (let i = 0; i < 40; i += 8) ctx.fillRect(dx + i, this.y, 4, 40); } else if (skin.pattern === 'rainbow') { ['#ff0000', '#ff7700', '#ffff00', '#00ff00', '#0077ff', '#7700ff'].forEach((c, i) => { ctx.fillStyle = c; ctx.fillRect(dx, this.y + i * 7, 40, 7); }); } else if (skin.pattern === 'neon') { ctx.shadowColor = skin.color; ctx.shadowBlur = 15; ctx.strokeRect(dx, this.y, 40, 40); ctx.shadowBlur = 0; } ctx.fillStyle = '#222'; ctx.fillRect(dx + 10, this.y + 10, 8, 8); ctx.fillRect(dx + 22, this.y + 10, 8, 8); }
...
// ==================== ГЕНЕРАЦИЯ УРОВНЯ ====================
function generateLevel(level){platforms=[];enemies=[];flyingEnemies=[];homelessEnemies=[];necromancers=[];vortexes=[];coins=[];powerUps=[];levelKeys=[];deadEnemiesForNecro=[];windProjectiles=[];roundCoins=0;roundDamage=0;lavaPlatforms=[];const minH=100,maxH=canvas.height-150;const pc=CONFIG.level.basePlatforms+Math.floor(level*CONFIG.level.platformGrowth);const ec=CONFIG.level.baseEnemies+Math.floor(level*CONFIG.level.enemyGrowth);const fc=Math.max(0,Math.floor(level/3));levelWidth=CONFIG.level.baseWidth+(level-1)*CONFIG.level.widthGrowth;platforms.push(new Platform(80,canvas.height-200,180,0));let lx=100,ly=canvas.height-250,dir=1;for(let i=0;i<pc;i++){const w=70+Math.random()*130,mig=120+(level*10),mag=220+(level*15);const x=lx+mig+Math.random()*(mag-mig);if(Math.random()>0.6)dir*=-1;const yc=80+Math.random()*100;let y=Math.max(minH,Math.min(maxH,ly+dir*yc));platforms.push(new Platform(x,y,w,Math.floor(Math.random()*platTex.length)));if(i>2&&i<pc-2&&Math.random()<0.4){const cc=Math.floor(Math.random()*3)+1;for(let j=0;j<cc;j++)coins.push({x:x+20+j*25,y:y-30,size:12,color:'#FFDE7D',bounce:Math.random()*Math.PI*2});}lx=x;ly=y;}platforms.push(new Platform(levelWidth-200,canvas.height-200,200,5));const kp=platforms[Math.floor(Math.random()*(platforms.length-3))+2];levelKeys.push({x:kp.x+kp.width/2-15,y:kp.y-45,size:28,collected:false,floatOffset:0});if(level>=3){const safePlat=platforms[Math.floor(Math.random()*(platforms.length-5))+3];const lp=new Platform(safePlat.x-30,safePlat.y,110,Math.floor(Math.random()*platTex.length),true);platforms.push(lp);lavaPlatforms.push(lp);homelessEnemies.push(new HomelessEnemy(safePlat.x+15,safePlat.y-40,lp));platforms.push(new Platform(safePlat.x+90,safePlat.y-80,70,Math.floor(Math.random()*platTex.length)));}const newTypes=['necromancer','vortex'];for(let i=0;i<ec;i++){const pi=Math.floor(Math.random()*(platforms.length-5))+3;const p=platforms[pi];if(level>=3&&i>0&&i%4===0){const t=newTypes[Math.floor(Math.random()*newTypes.length)];if(t==='necromancer')necromancers.push(new NecromancerEnemy(p.x+p.width/2-20,p.y-40));else vortexes.push(new VortexEnemy(p.x+p.width/2-20,150+Math.random()*200));}else enemies.push(new Enemy(p.x+p.width/2-20,p.y-40,Math.floor(Math.random()*3)));}for(let i=0;i<fc;i++)flyingEnemies.push(new FlyingEnemy(300+Math.random()*(levelWidth-600),100+Math.random()*200));for(let i=0;i<Math.floor(level/2)+1;i++){const pi=Math.floor(Math.random()*(platforms.length-10))+5;const p=platforms[pi];powerUps.push({x:p.x+p.width/2,y:p.y-40,size:15,color:'#FF2E63',type:'health'});}for(let cx=800;cx<levelWidth-300;cx+=800)platforms.push(new Platform(cx,canvas.height-180,100,2));return levelWidth;}
...
// ==================== ОСНОВНОЙ ЦИКЛ ====================
function gameLoop(){if(!gameRunning)return;player.update(keys);updateCamera();updateCoins();updateKeys();updatePowerUps();updateArrows();updateWindProjectiles();updateNewEnemies();decayCombo();checkEnemyCollisions();checkCheckpoints();if(boss)boss.update();if(player.x>levelWidth-100&&(!boss||!boss.active)){completeLevel();return;}if(screenShake>0){screenShake--;ctx.setTransform(1,0,0,1,(Math.random()-0.5)*shakeIntensity,(Math.random()-0.5)*shakeIntensity);}ctx.clearRect(0,0,canvas.width,canvas.height);drawBackground();platforms.forEach(p=>{p.update();p.draw(ctx);});enemies.forEach(e=>{e.update();e.draw(ctx);});flyingEnemies.forEach(e=>{e.update();e.draw(ctx);});necromancers.forEach(e=>e.draw(ctx));vortexes.forEach(e=>e.draw(ctx));homelessEnemies.forEach(e=>e.draw(ctx));if(boss)boss.draw(ctx);drawKeys();drawCoins();drawPowerUps();drawArrows();drawWindProjectiles();drawParticles();player.draw(ctx,cameraX);drawGoal();ctx.setTransform(1,0,0,1,0,0);updateDashIndicator();requestAnimationFrame(gameLoop);}

function completeLevel(){gameRunning=false;AudioSys.levelComplete();addScore(1000*currentLevel);const er=calculateEloChange();document.getElementById('levelScore').textContent=Math.floor(1000*currentLevel*comboMultiplier);document.getElementById('maxCombo').textContent=`x${maxCombo}`;document.getElementById('coinsCollected').textContent=roundCoins;document.getElementById('damageTaken').textContent=roundDamage;const el=document.getElementById('eloChangeDisplay');el.textContent=(er.change>=0?'+':'')+er.change;el.style.color=er.change>=0?'#4af626':'#ff2e63';document.getElementById('levelComplete').style.display='flex';showEloChange(er.change);for(let i=0;i<80;i++)particlePool.acquire(player.x+20,player.y+20,platTex[Math.floor(Math.random()*platTex.length)].c);setTimeout(()=>{document.getElementById('levelComplete').style.display='none';currentLevel++;generateLevel(currentLevel);player.reset();playerHealth=maxHealth;updateHealthBar();cameraX=0;comboCount=1;comboMultiplier=1;maxCombo=1;lastCheckpointX=0;updateUI();updateDashIndicator();updateEloDisplay();gameRunning=true;gameLoop();},2500);}
