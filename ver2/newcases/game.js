...
const bossTextures = {
    ...
    getRandom(colors) {
        return Math.random() < 0.7 ? { type: 'color', color: colors[Math.floor(Math.random() * colors.length)] } : { type: 'image', image: this.images[Math.floor(Math.random() * this.images.length)] };
    }
};
...
function closeShop() {
    ...
    gameRunning = true; if (typeof gameLoop === 'function') gameLoop();
}
...
function openChestType(id) {
    ...
    setTimeout(() => {
        ...
        if (has) {
            totalKeys += Math.floor(ch.price * 0.2); saveAllData();
            document.getElementById('shopKeyCount').textContent = totalKeys;
...
function buyClass(id, price) {
    ...
    if (confirm(`Купить ${getClassData(id).name} за ${price} 🔑?`)) {
        totalKeys -= price; unlockedClasses.push(id); equippedClass = id; saveAllData(); applyClassStats(); renderClasses();
        document.getElementById('shopKeyCount').textContent = totalKeys;
...
function showAuraEffectOnPlayer(x, y, aura) {
    ...
    if (aura.id === 'batidao_aura' && batidaoImage) {
        e.style.background = `radial-gradient(circle,${aura.effectColor},transparent) url(${batidaoImage.src}) center/cover overlay`;
        e.style.boxShadow = `0 0 50px ${aura.color}`; e.style.animation = 'batidaoAuraPlayer 0.5s ease-out forwards';
        for (let i = 0; i < 30; i++) {
            setTimeout(() => {
        ...
        e.style.background = `radial-gradient(circle,${aura.effectColor},transparent)`; e.style.boxShadow = `0 0 40px ${aura.color}`;
...
function loadBatidaoImage() {
    ...
    img.onload = () => { batidaoImage = img; }; img.onerror = () => { batidaoImage = null; };
...
class Player {
    ...
    update(k) {
        ...
        if ((k['Shift'] || k['shift'] || k['q'] || k['й']) && this.dashCharges > 0 && this.dashCooldown <= 0) {
            this.startDash(); k['Shift'] = k['shift'] = k['q'] = k['й'] = false;
        }
        if (equippedClass === 'archer' && (k['v'] || k['V'] || k['м'] || k['М']) && arrowCooldown <= 0) {
            this.shootArrow(); arrowCooldown = 25; k['v'] = k['V'] = k['м'] = k['М'] = false;
        }
    ...
    meleeAttack() {
        ...
        if (hit) {
            for (let i = 0; i < 25; i++) particlePool.acquire(cx + (Math.random() - 0.5) * 40, cy + (Math.random() - 0.5) * 40, '#ffaa33');
            shakeScreen(3);
        }
    }
    ...
    checkPlatformCollisions() {
        ...
        for (let p of platforms) {
            if (this.x < p.x + p.width && this.x + this.width > p.x && this.y + this.height > p.y && this.y + this.height < p.y + 15 && this.velY >= 0) {
                this.y = p.y - this.height; this.velY = 0;
                if (this.jumping) { this.createLandParticles(); this.jumping = false; }
                this.jumpCount = 0; on = true;
                if (p.type === 'breaking' && !p.broken) p.breakTimer = 60;
            }
        }
        if (on) this.jumpCount = 0;
    }
    checkLavaDamage() {
    ...
    draw(ctx, cx) {
        ...
        if (skin.type === 'sphere') {
            ...
            // === КЛАССИЧЕСКИЙ КУБ (ИСПРАВЛЕНО: используется skin.color, а не класс) ===
...
class Boss {
    constructor(x, y) {
        ...
        this.maxHealth = CONFIG.boss.health + Math.floor(currentLevel / 5) * 5; this.health = this.maxHealth;
        this.speed = CONFIG.boss.moveSpeed; this.dir = 1; this.cd = 0; this.active = true;
        this.tex = bossTextures.getRandom(enCol); this.phase = 0; this.mp = 0; this.sp = 0;
        this.color = this.tex.type === 'color' ? this.tex.color : '#ff0000';
    }
    update() {
        if (!this.active) return;
        this.mp += 0.02; this.cd--;
        if (Math.abs(this.x - player.x) > 200) { this.x += this.speed * this.dir; if (this.x < cameraX + 100 || this.x > cameraX + canvas.width - 140) this.dir *= -1; }
        this.y += Math.sin(this.mp) * 2;
        if (this.cd <= 0) { this.attack(); this.cd = 120 - (currentLevel * 2); }
        if (player.checkCollision(this)) { if (player.takeDamage(30, this.x < player.x ? 15 : -15, -10, '#ff0000')) return; }
    }
    attack() {
        this.sp = (this.sp + 1) % 3;
        if (this.sp === 0) for (let i = 0; i < 15; i++) particlePool.acquire(this.x + 20, this.y + 20, '#ff0000');
        else if (this.sp === 1) for (let i = -1; i <= 1; i++) particlePool.acquire(this.x + 20, this.y + 20, 0, 4, 10, '#ff6600', false);
        else for (let i = 0; i < 8; i++) { const a = (i / 8) * Math.PI * 2; particlePool.acquire(this.x + 20, this.y + 20, Math.cos(a) * 6, Math.sin(a) * 6, 8, '#ff0000'); }
        for (let i = 0; i < 15; i++) particlePool.acquire(this.x + 20, this.y + 20, '#ff0000');
    }
    takeDamage(d = 1) {
        this.health -= d; AudioSys.bossHit();
        for (let i = 0; i < 10; i++) particlePool.acquire(this.x + Math.random() * 40, this.y + Math.random() * 40, '#ff0000');
        if (this.health <= 0) { this.defeat(); return true; } updateBossHealth(); return false;
    }
    defeat() {
        this.active = false; AudioSys.bossDefeat(); bossesDefeated++;
        for (let i = 0; i < 100; i++) particlePool.acquire(this.x + 20, this.y + 20, this.color);
        addScore(5000 * comboMultiplier); updateCombo();
        document.getElementById('bossHealthBar').style.display = 'none';
        if (Math.random() < 0.8) powerUps.push({ x: this.x + 20, y: this.y + 20, size: 20, color: '#FF2E63', type: 'health' });
        if (Math.random() < 0.5) powerUps.push({ x: this.x + 60, y: this.y + 20, size: 20, color: '#FFDE7D', type: 'dash' });
        shakeScreen(10);
    }
    draw(ctx) {
        if (!this.active) return;
        const dx = this.x - cameraX; ctx.shadowColor = '#ff0000'; ctx.shadowBlur = 30;
        if (this.tex.type === 'image' && this.tex.image) try { ctx.drawImage(this.tex.image, dx, this.y, 40, 40); } catch (e) { ctx.fillStyle = this.color; ctx.fillRect(dx, this.y, 40, 40); }
        else { ctx.fillStyle = this.color; ctx.fillRect(dx, this.y, 40, 40); ctx.fillStyle = 'rgba(0,0,0,0.3)'; for (let i = 0; i < 40; i += 20) for (let j = 0; j < 40; j += 20) if ((i / 20 + j / 20) % 2 === 0) ctx.fillRect(dx + i, this.y + j, 20, 20); }
        ctx.shadowBlur = 0; ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(dx + 10, this.y + 15, 5, 0, Math.PI * 2); ctx.arc(dx + 30, this.y + 15, 5, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = '#000'; ctx.beginPath(); ctx.arc(dx + 10 + (this.dir * 2), this.y + 15, 2, 0, Math.PI * 2); ctx.arc(dx + 30 + (this.dir * 2), this.y + 15, 2, 0, Math.PI * 2); ctx.fill();
        if (this.health < this.maxHealth * 0.3 && Math.floor(Date.now() / 200) % 2 === 0) { ctx.strokeStyle = '#ff0000'; ctx.lineWidth = 4; ctx.strokeRect(dx - 5, this.y - 5, 50, 50); }
    }
}

class Enemy {
    constructor(x, y, t = 0) {
        this.x = x; this.y = y; this.width = 40; this.height = 40; this.type = t;
        this.color = enCol[Math.floor(Math.random() * enCol.length)]; this.speed = t === 0 ? 0 : 1 + Math.random() * 2;
        this.dir = Math.random() > 0.5 ? 1 : -1; this.sc = 0; this.pr = 100 + Math.random() * 200;
        this.sx = x; this.jc = 0; this.health = CONFIG.enemies.baseHealth[t === 0 ? 'shooter' : t === 1 ? 'patrol' : 'jumper'];
        this.maxHealth = this.health; this.scoreValue = CONFIG.enemies.baseScore[t === 0 ? 'shooter' : t === 1 ? 'patrol' : 'jumper'];
        this.tex = Math.floor(Math.random() * 3); this.active = true;
    }
    update() {
        if (!this.active) return;
        if (this.type === 1) { this.x += this.speed * this.dir; if (Math.abs(this.x - this.sx) > this.pr) this.dir *= -1; }
        if (this.type === 2 && this.jc <= 0) {
            let on = false;
            for (let p of platforms) { if (this.x < p.x + p.width && this.x + this.width > p.x && this.y + this.height > p.y && this.y + this.height < p.y + 30) { on = true; break; } }
            if (on) { const d = player.x > this.x ? 1 : -1; this.y -= 12 + Math.random() * 4; this.x += d * (8 + Math.random() * 4); this.jc = 60 + Math.random() * 120; }
        }
        this.jc--; this.y += 0.8;
        for (let p of platforms) { if (this.x < p.x + p.width && this.x + this.width > p.x && this.y + this.height > p.y && this.y + this.height < p.y + 30) { this.y = p.y - this.height; if (this.type === 1 && (this.x <= p.x + 5 || this.x + this.width >= p.x + p.width - 5)) this.dir *= -1; } }
    }
    takeDamage(d = 1, s) { this.health -= d; if (this.health <= 0) { this.destroy(s); return true; } return false; }
    destroy(s) {
        this.active = false; if (s !== 'necro') deadEnemiesForNecro.push({ x: this.x, y: this.y, color: this.color, timer: 180 });
        addScore(this.scoreValue * comboMultiplier); updateCombo(); AudioSys.collect();
        for (let i = 0; i < 20; i++) particlePool.acquire(this.x + 20, this.y + 20, this.color);
        if (Math.random() < 0.35) coins.push({ x: this.x + 20, y: this.y + 20, size: 12, color: '#FFDE7D', bounce: 0 });
        if (Math.random() < 0.12) powerUps.push({ x: this.x + 20, y: this.y + 20, size: 15, color: '#FF2E63', type: 'health' });
    }
    draw(ctx) {
        if (!this.active) return;
        ctx.fillStyle = this.color; ctx.fillRect(this.x - cameraX, this.y, 40, 40);
        ctx.fillStyle = 'rgba(0,0,0,0.25)';
        if (this.tex === 0) { for (let i = 0; i < 40; i += 15) ctx.fillRect(this.x - cameraX + i, this.y, 8, 40); }
        else if (this.tex === 1) { for (let i = 0; i < 40; i += 10) for (let j = 0; j < 40; j += 10) if ((Math.floor(i / 10) + Math.floor(j / 10)) % 2 === 0) ctx.fillRect(this.x - cameraX + i, this.y + j, 10, 10); }
        else { ctx.beginPath(); ctx.arc(this.x - cameraX + 20, this.y + 20, 10, 0, Math.PI * 2); ctx.fill(); }
        ctx.fillStyle = '#000'; ctx.fillRect(this.x - cameraX + 10, this.y + 10, 8, 8); ctx.fillRect(this.x - cameraX + 22, this.y + 10, 8, 8);
        if (this.health < this.maxHealth) { ctx.fillStyle = '#4af626'; ctx.fillRect(this.x - cameraX, this.y - 8, (40 * this.health) / this.maxHealth, 4); }
    }
}

class FlyingEnemy {
    constructor(x, y) {
        this.x = x; this.y = y; this.oy = y; this.width = 40; this.height = 40;
        this.color = flyCol[Math.floor(Math.random() * flyCol.length)]; this.speed = 1.5 + Math.random() * 1.5;
        this.dir = Math.random() > 0.5 ? 1 : -1; this.health = 2; this.maxHealth = 2;
        this.wp = 0; this.fa = 20 + Math.random() * 30; this.fs = 0.05 + Math.random() * 0.03;
        this.fp = Math.random() * Math.PI * 2; this.cc = 0; this.ic = false; this.cd = 0; this.active = true;
    }
    update() {
        if (!this.active) return; this.wp += 0.2; this.fp += this.fs; this.y = this.oy + Math.sin(this.fp) * this.fa;
        this.x += this.speed * this.dir; if (this.x < cameraX - 50 || this.x > cameraX + canvas.width + 50) this.dir *= -1;
        if (!this.ic) { this.cc--; if (this.cc <= 0) { const dx = player.x - this.x, dy = player.y - this.y; if (Math.sqrt(dx * dx + dy * dy) < 300) { this.ic = true; this.cd = dx > 0 ? 1 : -1; this.cc = 120 + Math.random() * 120; } } }
        else { this.x += 8 * this.cd; this.y += (player.y - this.y) * 0.1; if (Math.abs(player.x - this.x) < 50 || this.x < cameraX - 100 || this.x > cameraX + canvas.width + 100) { this.ic = false; this.cc = 60 + Math.random() * 60; } }
    }
    takeDamage(d = 1, s) { this.health -= d; if (this.health <= 0) { this.destroy(s); return true; } return false; }
    destroy(s) {
        this.active = false; if (s !== 'necro') deadEnemiesForNecro.push({ x: this.x, y: this.y, color: this.color, timer: 180 });
        addScore(200 * comboMultiplier); updateCombo(); AudioSys.collect();
        for (let i = 0; i < 30; i++) particlePool.acquire(this.x + 20, this.y + 20, this.color);
        if (Math.random() < 0.5) coins.push({ x: this.x + 20, y: this.y + 20, size: 12, color: '#FFDE7D', bounce: 0 });
        if (Math.random() < 0.3) powerUps.push({ x: this.x + 20, y: this.y + 20, size: 15, color: '#FF2E63', type: 'health' });
    }
    draw(ctx) {
        if (!this.active) return; ctx.fillStyle = this.color; ctx.fillRect(this.x - cameraX, this.y, 40, 40);
        ctx.fillStyle = this.color + '80'; const wy = this.y + 20, wa = Math.sin(this.wp) * 10;
        ctx.beginPath(); ctx.moveTo(this.x - cameraX - 15, wy); ctx.quadraticCurveTo(this.x - cameraX - 25, wy - wa, this.x - cameraX - 15, wy); ctx.fill();
        ctx.beginPath(); ctx.moveTo(this.x - cameraX + 55, wy); ctx.quadraticCurveTo(this.x - cameraX + 65, wy - wa, this.x - cameraX + 55, wy); ctx.fill();
        ctx.fillStyle = '#000'; ctx.fillRect(this.x - cameraX + 8, this.y + 8, 6, 6); ctx.fillRect(this.x - cameraX + 26, this.y + 8, 6, 6);
        ctx.shadowColor = this.color; ctx.shadowBlur = 15; ctx.fillRect(this.x - cameraX, this.y, 40, 40); ctx.shadowBlur = 0;
        if (this.health < this.maxHealth) { ctx.fillStyle = '#4af626'; ctx.fillRect(this.x - cameraX, this.y - 8, (40 * this.health) / this.maxHealth, 4); }
    }
}

class NecromancerEnemy {
    constructor(x, y) { this.x = x; this.y = y; this.width = 40; this.height = 40; this.color = '#5d4a66'; this.health = 3; this.maxHealth = 3; this.active = true; this.sc = 0; }
    update() {
        if (!this.active) return; this.sc--;
        if (Math.abs(this.x - player.x) < 300) this.x += (player.x > this.x ? 0.5 : -0.5);
        if (this.sc <= 0) {
            for (let d of deadEnemiesForNecro) {
                if (d.timer > 0 && Math.hypot(d.x - this.x, d.y - this.y) < 200) {
                    d.timer--;
                    if (d.timer <= 0) {
                        const n = new Enemy(d.x, d.y, 0); enemies.push(n);
                        deadEnemiesForNecro = deadEnemiesForNecro.filter(x => x !== d);
                        AudioSys.necroSummon(); for (let i = 0; i < 20; i++) particlePool.acquire(d.x, d.y, '#8e44ad'); break;
                    }
                }
            }
            this.sc = 120;
        ...
        this.y += 0.6;
        for (let p of platforms) { if (this.x < p.x + p.width && this.x + this.width > p.x && this.y + this.height > p.y && this.y + this.height < p.y + 30) { this.y = p.y - this.height; break; } }
    }
    takeDamage(d = 1) { this.health -= d; if (this.health <= 0) { this.destroy(); return true; } return false; }
    destroy() { this.active = false; addScore(300 * comboMultiplier); updateCombo(); AudioSys.collect(); for (let i = 0; i < 30; i++) particlePool.acquire(this.x + 20, this.y + 20, '#8e44ad'); }
    draw(ctx) {
        if (!this.active) return; const dx = this.x - cameraX;
        ctx.fillStyle = this.color; ctx.fillRect(dx, this.y, 40, 40);
        ctx.fillStyle = '#3d2a4a'; ctx.beginPath(); ctx.moveTo(dx + 20, this.y - 15); ctx.lineTo(dx - 5, this.y + 10); ctx.lineTo(dx + 45, this.y + 10); ctx.fill();
        ctx.fillStyle = '#ff00ff'; ctx.beginPath(); ctx.arc(dx + 12, this.y + 15, 4, 0, Math.PI * 2); ctx.arc(dx + 28, this.y + 15, 4, 0, Math.PI * 2); ctx.fill();
        if (this.sc < 60) { ctx.strokeStyle = '#8e44ad'; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(dx + 20, this.y + 20, 40 + Math.sin(Date.now() / 100) * 10, 0, Math.PI * 2); ctx.stroke(); }
        if (this.health < this.maxHealth) { ctx.fillStyle = '#4af626'; ctx.fillRect(dx, this.y - 12, (40 * this.health) / this.maxHealth, 4); }
        // === ВРЕМЕННАЯ ПОДПИСЬ (УДАЛИТЬ ЧЕРЕЗ ДЕНЬ) ===
        ctx.fillStyle = '#fff'; ctx.font = 'bold 11px Unbounded'; ctx.textAlign = 'center'; ctx.fillText('НЕКРОМАНТ', dx + 20, this.y - 20);
    }
}

class VortexEnemy {
    constructor(x, y) { this.x = x; this.y = y; this.oy = y; this.width = 40; this.height = 40; this.color = '#00ffff'; this.health = 4; this.maxHealth = 4; this.active = true; this.rot = 0; this.fp = Math.random() * Math.PI * 2; }
    update() {
    ...
    draw(ctx) {
        ...
        ctx.restore(); ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(dx + 20, this.y + 20, 6, 0, Math.PI * 2); ctx.fill();
        if (this.health < this.maxHealth) { ctx.fillStyle = '#4af626'; ctx.fillRect(dx, this.y - 12, (40 * this.health) / this.maxHealth, 4); }
        // === ВРЕМЕННАЯ ПОДПИСЬ (УДАЛИТЬ ЧЕРЕЗ ДЕНЬ) ===
...
class HomelessEnemy {
    constructor(x, y, plat) { this.x = x; this.y = y; this.width = 40; this.height = 40; this.color = '#8B7355'; this.health = 5; this.maxHealth = 5; this.active = true; this.plat = plat; this.bw = 0; }
    update() { if (!this.active) return; this.bw += 0.15; this.y = this.plat.y - this.height; this.x = this.plat.x + this.plat.width / 2 - 20; }
    takeDamage(d = 1, s) { this.health -= (s === 'arrow' ? d * 1.67 : d); if (this.health <= 0) { this.destroy(); return true; } return false; }
    destroy() { this.active = false; addScore(180 * comboMultiplier); updateCombo(); AudioSys.collect(); for (let i = 0; i < 25; i++) particlePool.acquire(this.x + 20, this.y + 20, '#8B7355'); if (Math.random() < 0.15) coins.push({ x: this.x + 20, y: this.y - 20, size: 12, color: '#FFDE7D', bounce: 0 }); }
    draw(ctx) {
        if (!this.active) return; const dx = this.x - cameraX;
        ctx.fillStyle = this.color; ctx.fillRect(dx, this.y + 5, 40, 35);
        ctx.fillStyle = '#D2B48C'; ctx.beginPath(); ctx.arc(dx + 20, this.y + 10, 12, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = '#6B5443'; ctx.beginPath(); ctx.moveTo(dx + 8, this.y + 12); for (let i = 0; i < 6; i++) ctx.quadraticCurveTo(dx + 12 + i * 4, this.y + 20 + Math.sin(this.bw + i) * 3, dx + 14 + i * 4, this.y + 12); ctx.lineTo(dx + 32, this.y + 12); ctx.fill();
        ctx.fillStyle = '#000'; ctx.beginPath(); ctx.arc(dx + 15, this.y + 8, 3, 0, Math.PI * 2); ctx.arc(dx + 25, this.y + 8, 3, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = '#4a3728'; ctx.fillRect(dx + 5, this.y - 2, 30, 8); ctx.fillRect(dx + 2, this.y - 6, 36, 4);
        if (this.health < this.maxHealth) { ctx.fillStyle = '#4af626'; ctx.fillRect(dx, this.y - 15, (40 * this.health) / this.maxHealth, 4); }
        // === ВРЕМЕННАЯ ПОДПИСЬ (УДАЛИТЬ ЧЕРЕЗ ДЕНЬ) ===
        ctx.fillStyle = '#fff'; ctx.font = 'bold 11px Unbounded'; ctx.textAlign = 'center'; ctx.fillText('БОМЖ', dx + 20, this.y - 22);
    }
}

class Platform {
    constructor(x, y, w, ti, lv = false) {
        this.x = x; this.y = y; this.width = w; this.height = 25; this.tex = platTex[ti];
        this.glow = Math.random() > 0.7; this.gp = Math.random() * Math.PI * 2;
        this.type = Math.random() > 0.8 ? (Math.random() > 0.5 ? 'moving' : 'breaking') : 'normal';
        this.md = 1; this.ms = 1; this.ox = x; this.bt = 0; this.broken = false; this.isLava = lv;
    }
    update() {
        if (this.glow) this.gp += 0.05;
        if (this.type === 'moving' && !this.broken) { this.x += this.ms * this.md; if (Math.abs(this.x - this.ox) > 100) this.md *= -1; }
        if (this.type === 'breaking' && this.bt > 0) { this.bt--; if (this.bt === 0) { this.broken = true; setTimeout(() => this.broken = false, 3000); } }
    }
    draw(ctx) {
        if (this.broken) return;
        ctx.fillStyle = 'rgba(0,0,0,0.25)'; ctx.fillRect(this.x - cameraX + 3, this.y + 3, this.width, this.height);
        if (this.isLava) {
            ctx.fillStyle = '#ff4500'; ctx.fillRect(this.x - cameraX, this.y + 5, this.width, this.height - 5);
            ctx.fillStyle = '#ff6600'; for (let i = 0; i < this.width; i += 15) { ctx.beginPath(); ctx.arc(this.x - cameraX + i + 7, this.y + 10, 4, 0, Math.PI); ctx.fill(); }
        } else {
            ctx.fillStyle = this.tex.c; if (this.type === 'breaking' && this.bt > 0) ctx.globalAlpha = Math.sin(Date.now() / 100) * 0.3 + 0.7;
            ctx.fillRect(this.x - cameraX, this.y, this.width, this.height); ctx.globalAlpha = 1;
            ctx.fillStyle = 'rgba(255,255,255,0.12)'; const p = this.tex.p;
            ...
            else if (p === 'waves') { for (let i = 0; i < this.width; i += 30) { ctx.beginPath(); ctx.moveTo(this.x - cameraX + i, this.y + this.height / 2); for (let j = 0; j < 30; j += 5) { const x = this.x - cameraX + i + j, y = this.y + this.height / 2 + Math.sin(j / 30 * Math.PI * 2) * 5; ctx.lineTo(x, y); } ctx.lineTo(this.x - cameraX + i + 30, this.y + this.height); ctx.lineTo(this.x - cameraX + i, this.y + this.height); ctx.fill(); } }
        }
        if (this.glow && !this.isLava) { const gi = Math.sin(this.gp) * 0.3 + 0.7; ctx.shadowColor = this.tex.c; ctx.shadowBlur = 15 * gi; ctx.strokeStyle = this.tex.c; ctx.lineWidth = 2; ctx.strokeRect(this.x - cameraX, this.y, this.width, this.height); ctx.shadowBlur = 0; }
        ctx.strokeStyle = 'rgba(0,0,0,0.25)'; ctx.lineWidth = 2; ctx.strokeRect(this.x - cameraX, this.y, this.width, this.height);
        if (this.type === 'breaking') { ctx.fillStyle = '#ff0000'; for (let i = 0; i < 3; i++) { ctx.beginPath(); ctx.arc(this.x - cameraX + this.width / 4 + i * (this.width / 4), this.y + 5, 3, 0, Math.PI * 2); ctx.fill(); } }
        if (this.type === 'moving') { ctx.fillStyle = '#0088ff'; const ac = Math.floor(this.width / 30); for (let i = 0; i < ac; i++) { const ax = this.x - cameraX + 15 + i * 30; ctx.beginPath(); ctx.moveTo(ax, this.y + this.height / 2); ctx.lineTo(ax + 10 * this.md, this.y + 10); ctx.lineTo(ax + 10 * this.md, this.y + this.height - 10); ctx.closePath(); ctx.fill(); } }
    }
}
...
function generateLevel(level) {
    ...
    // Гарантированный спавн бомжа на уровне 3+
    if (level >= 3) {
        ...
        platforms.push(lp); lavaPlatforms.push(lp);
        homelessEnemies.push(new HomelessEnemy(safePlat.x + 15, safePlat.y - 40, lp));
        platforms.push(new Platform(safePlat.x + 90, safePlat.y - 80, 70, Math.floor(Math.random() * platTex.length)));
    }
...
function gameLoop() {
    ...
    player.update(keys); updateCamera(); updateCoins(); updateKeys(); updatePowerUps(); updateArrows(); updateNewEnemies();
    decayCombo(); checkEnemyCollisions(); checkCheckpoints(); if (boss) boss.update();
...
function completeLevel() {
    ...
    setTimeout(() => {
        document.getElementById('levelComplete').style.display = 'none'; currentLevel++; generateLevel(currentLevel);
        player.reset(); playerHealth = maxHealth; updateHealthBar(); cameraX = 0; comboCount = 1; comboMultiplier = 1; maxCombo = 1; lastCheckpointX = 0;
