...
// Биомы
const BIOMES = {
    BLOCKY: { name: 'Блочный', color: '#4af626', bgStart: '#0a0a1a', bgEnd: '#1a1a2e', platformColors: ['#FF2E63', '#08D9D6', '#FFDE7D', '#6A2C70', '#4ECDC4', '#FF9A76'], patterns: true, special: null },
    MOUNTAIN: { name: 'Горы', color: '#8B8B8B', bgStart: '#2c2c3e', bgEnd: '#4a4a5e', platformColors: ['#7C7C7C', '#5C5C5C', '#9C9C9C', '#6C6C6C'], patterns: false, special: 'steep' },
    CAVE: { name: 'Пещера', color: '#6B8E6B', bgStart: '#0a0a0a', bgEnd: '#1a1a1a', platformColors: ['#4A5A4A', '#3A4A3A', '#5A6A5A', '#2A3A2A'], patterns: false, special: 'dark' },
    FACTORY: { name: 'Завод', color: '#B87333', bgStart: '#1a1a1a', bgEnd: '#2a2a2a', platformColors: ['#8B7355', '#A0522D', '#6B4226', '#CD853F', '#D2691E'], patterns: false, special: 'industrial' }
};
...
function updateDashIndicator(){const dots=document.querySelectorAll('.dash-dot');if(player) dots.forEach((dot,i)=>{dot.classList.toggle('active',i<player.dashCharges);});}
...
function getBiomeForLevel(level) {
    const biomes = ['BLOCKY', 'MOUNTAIN', 'CAVE', 'FACTORY'];
    return biomes[(level - 1) % biomes.length];
}

function getBiomeColor(biome) {
    return BIOMES[biome]?.color || '#4af626';
}

function generateDecorations(biome, x, y, width) {
    const decorations = [];
    if (biome === 'CAVE') {
        // Сталактиты и растительность
        for (let i = 0; i < width/50; i++) {
            if (Math.random() < 0.3) {
                decorations.push({
                    type: 'stalactite',
                    x: x + i * 50 + Math.random() * 30,
        ...
        // Снежные шапки и камни
        for (let i = 0; i < width/40; i++) {
            if (Math.random() < 0.2) {
                decorations.push({
                    type: 'rock',
                    x: x + i * 40 + Math.random() * 30,
                    y: y - 5,
...
class Boss { 
    ...
    performAttack() {
        ...
        switch(this.shootPattern){
            case 0:
                for(let i=0;i<20;i++) {
                    particlePool.acquire(cx + (Math.random()-0.5)*50, cy + (Math.random()-0.5)*50, '#ff0000');
                }
                break;
            case 1:
                for(let i=-1;i<=1;i++) {
                    for(let j=0;j<8;j++) {
                        particlePool.acquire(cx + i*30, cy + (Math.random()-0.5)*40, '#ff6600');
                    }
                }
                break;
            case 2:
                for(let i=0;i<12;i++) {
                    ...
                    particlePool.acquire(px, py, '#ff4400');
                }
                break;
        }
        for(let i=0;i<15;i++) {
            particlePool.acquire(cx + (Math.random()-0.5)*60, cy + (Math.random()-0.5)*60, '#ff0000');
        }
    }
    ...
    takeDamage(amount = 1) {
        ...
        AudioSys.bossHit();
        for(let i=0;i<15;i++) {
            particlePool.acquire(this.x+Math.random()*this.width, this.y+Math.random()*this.height, '#ff0000');
        }
        if(this.health <= 0){
            this.defeat();
            return true;
    ...
    defeat() {
        ...
        bossesDefeated++;
        for(let i=0;i<100;i++) {
            particlePool.acquire(this.x+this.width/2, this.y+this.height/2, this.color);
        }
...
class Platform { 
    constructor(x,y,width,textureIndex, biome){
        this.x=x;this.y=y;this.width=width;this.height=25;
        this.biome = biome;
        const biomeColors = BIOMES[biome]?.platformColors || BIOMES.BLOCKY.platformColors;
        this.texture = {
            color: biomeColors[textureIndex % biomeColors.length],
            pattern: BIOMES[biome]?.patterns ? ['stripes', 'dots', 'checker', 'zigzag', 'bricks', 'waves'][textureIndex % 6] : 'solid'
        };
        this.hasGlow=Math.random()>0.7 && biome !== 'CAVE';
        ...
        this.breakTimer=0;
        this.broken=false;
        this.decorations = generateDecorations(biome, x, y, width);
    } 
    update(){
    ...
    draw(ctx){
        ...
        for(let d of this.decorations) {
            if(d.type === 'stalactite') {
                ...
                ctx.beginPath();
                ctx.moveTo(this.x-cameraX + d.x - this.x + d.width/2, d.y);
                ctx.lineTo(this.x-cameraX + d.x - this.x, d.y + d.height);
                ctx.lineTo(this.x-cameraX + d.x - this.x + d.width, d.y + d.height);
                ctx.fill();
            } else if(d.type === 'mushroom') {
                ctx.fillStyle = '#8B6B8B';
                ctx.beginPath();
                ctx.rect(this.x-cameraX + d.x - this.x - 3, d.y - d.size/2, 6, d.size);
                ctx.fill();
                ctx.fillStyle = '#B87CB8';
                ctx.beginPath();
                ctx.arc(this.x-cameraX + d.x - this.x, d.y - d.size/2, d.size/2, 0, Math.PI*2);
                ctx.fill();
            } else if(d.type === 'gear') {
                ctx.fillStyle = '#B87333';
                ctx.beginPath();
                ctx.arc(this.x-cameraX + d.x - this.x, d.y, d.size/2, 0, Math.PI*2);
                ctx.fill();
                for(let k=0;k<8;k++) {
                    const angle = k * Math.PI/4;
                    ctx.fillRect(this.x-cameraX + d.x - this.x + Math.cos(angle)*d.size/2 - 3, d.y + Math.sin(angle)*d.size/2 - 3, 6, 6);
                }
            } else if(d.type === 'pipe') {
                ctx.fillStyle = '#8B7355';
                ctx.fillRect(this.x-cameraX + d.x - this.x, d.y, d.width, d.height);
                ctx.fillStyle = '#A0522D';
                for(let k=0;k<3;k++) {
                    ctx.fillRect(this.x-cameraX + d.x - this.x + 3 + k*3, d.y + 5, 2, d.height-10);
                }
            } else if(d.type === 'rock') {
                ctx.fillStyle = '#7C7C7C';
                ctx.beginPath();
                ctx.ellipse(this.x-cameraX + d.x - this.x, d.y, d.size/2, d.size/3, 0, 0, Math.PI*2);
                ctx.fill();
            }
        }
...
function generateLevel(level){ 
    ...
    // Стартовая платформа
    platforms.push(new Platform(80,canvas.height-200,180,0, biome));
    
    let lastX=100,lastY=canvas.height-250,direction=1;
    
    for(let i=0;i<platformCount;i++){ 
        let width=70+Math.random()*130;
        let minXGap=120+(level*10);
        let maxXGap=220+(level*15);
        
        // Особенности биомов
        if(biome === 'MOUNTAIN') {
            width = 50 + Math.random()*80;
            minXGap = 80 + level*5;
            maxXGap = 150 + level*10;
        } else if(biome === 'CAVE') {
        ...
        let yChange=80+Math.random()*100;
        if(biome === 'MOUNTAIN') yChange = 40+Math.random()*150;
        if(biome === 'CAVE') yChange = 60+Math.random()*80;
        
        let y=Math.max(minHeight,Math.min(maxHeight,lastY+direction*yChange));
        
        platforms.push(new Platform(x,y,width,Math.floor(Math.random()*6), biome));
    ...
    platforms.push(new Platform(levelWidth-200,canvas.height-200,200,5, biome));
    ...
    if(isBossLevel){ 
        for(let i=0;i<Math.floor(enemyCount/2);i++){
            ...
            if(p) enemies.push(new Enemy(p.x+p.width/2-20,p.y-40,Math.floor(Math.random()*3)));
        }
    ...
    for(let i=0;i<Math.floor(level/2)+1;i++){
        ...
        const p=platforms[pi];
        if(p) powerUps.push({x:p.x+p.width/2,y:p.y-40,size:15,color:'#FF2E63',type:'health'});
    }
    for(let cx=800;cx<levelWidth-300;cx+=800) platforms.push(new Platform(cx,canvas.height-180,100,2, biome));
...
function drawBackground(){
    ...
    grad.addColorStop(0,biome.bgStart);
    grad.addColorStop(1,biome.bgEnd);
    ctx.fillStyle=grad;
    ctx.fillRect(0,0,canvas.width,canvas.height);
    
    // Звёзды или пыль в зависимости от биома
    ...
    // Дополнительный декор для пещеры
    if(currentBiome === 'CAVE') {
        ctx.fillStyle = 'rgba(30,30,30,0.3)';
        for(let i=0;i<20;i++) {
            ctx.beginPath();
            ctx.ellipse((i*150 + cameraX*0.2) % canvas.width, canvas.height-50, 40, 20, 0, 0, Math.PI*2);
            ctx.fill();
        }
    }
    
    // Индустриальный смог для завода
    if(currentBiome === 'FACTORY') {
        ctx.fillStyle = 'rgba(100,70,40,0.1)';
        for(let i=0;i<30;i++) {
            ctx.beginPath();
            ctx.rect((i*80 + cameraX*0.3) % canvas.width, canvas.height-100, 30, 100);
            ctx.fill();
        }
    }
}
...
function drawGoal(){
    ...
    if(gx<canvas.width+100&&gx>-100){
        ...
        grad.addColorStop(0,`${getBiomeColor(currentBiome)}aa`);
        grad.addColorStop(1,`${getBiomeColor(currentBiome)}00`);
        ctx.fillStyle=grad;
        ctx.fillRect(gx-180,0,360,canvas.height);
        ctx.strokeStyle=getBiomeColor(currentBiome);
        ctx.lineWidth=5;
        ctx.beginPath();
        ctx.moveTo(gx,0);
...
function completeLevel(){
    ...
    for(let i=0;i<80;i++) particlePool.acquire(player.x+player.width/2,player.y+player.height/2,BIOMES[currentBiome].color);
    safeTimeout(()=>{
